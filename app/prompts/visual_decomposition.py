...
# v38 addendum: source-unit decision engine runs before this prompt. Therefore
# the units shown in cards_json are FINAL units, not a mechanical copy of the
# source image. If a card is marked replace_with_new, do not preserve the old
# source object under any circumstances.
VISUAL_DECOMPOSITION_SYSTEM_PROMPT += """

V38 SOURCE/FINAL UNIT RULE:
The cards you receive are FINAL units after medical/regional fact-checking.
Do not re-add source units that were removed or merged. Do not preserve old
objects for cards with source_policy=replace_with_new. For keep/merge units,
preserve only the useful components belonging to the selected final unit.
"""

# v39 Semantic-layout addendum. This prompt is appended rather than replacing the
# original so older callers remain compatible.
VISUAL_DECOMPOSITION_SYSTEM_PROMPT += """

V39 SEMANTIC-LAYOUT RULES:
Before extracting components, obey the SEMANTIC_LAYOUT_MAP if it is provided in
contract_summary. A semantic card is the authoritative logical unit. Do not split
one semantic card into multiple units because of slash-separated labels or
multiple object names. Do not combine components from different semantic cards.

For each final unit:
1. locate its matching semantic card;
2. decompose only components within that semantic card boundary;
3. preserve PRIMARY medical visual separately;
4. preserve SECONDARY context object separately;
5. remove labels, descriptions, backgrounds, UI/watermarks.

If a requested final unit cannot be matched to a semantic card and source_policy
is not replace_with_new/generate_new, mark generate_new rather than inventing a
wrong crop. Bbox coordinates must correspond to actual components, not salient
random image regions.
"""
