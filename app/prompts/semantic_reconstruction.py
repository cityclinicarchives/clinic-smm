...
7) Multi-visual card: если один пункт имеет 2+ смысловые иллюстрации (препарат+симптом, продукт+орган и т.п.), создай отдельные visual_entity/PNG и ОБЯЗАТЕЛЬНО свяжи их в той же карточке: primary_png_id=главный объект, secondary_png_id=поясняющий объект, png_ids=[оба]. Не оставляй visual_explanation PNG сиротами.
...
- Для списка вида «объект слева + симптом/орган справа» делай 1 карточку = 2 PNG: primary_png_id для объекта, secondary_png_id для симптома/органа.
- Если у карточки две смысловые картинки, выдели обе отдельными PNG и привяжи обе к одной карточке через primary_png_id/secondary_png_id/png_ids.
""".strip()
