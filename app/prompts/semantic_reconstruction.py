...
- Для extract_from_source укажи source_crop_hint.relative_box в координатах 0..1: [x1, y1, x2, y2]. Это должна быть область только вокруг смыслового visual-объекта без старой подписи, без заголовка, без соседних карточек и без интерфейса.
- Если точный crop невозможен, укажи source_crop_hint.confidence ниже 0.6 и объясни проблему.
...
- Для extract_from_source обязательно заполни quality_strategy и source_crop_hint.relative_box; crop должен исключать старый текст.
