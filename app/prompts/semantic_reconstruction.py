...
Задача: вернуть максимально КОМПАКТНЫЙ v43.4-plan реконструкции медицинской инфографики. План должен быть достаточным для:
...
- Do not rewrite category descriptions unless factually incorrect, unsafe or clearly misleading.
...
HERO VISUAL:
- Detect hero_visual: a non-repeating main image that explains the topic of the whole infographic. Preserve it separately from cards.
- Hero visual is not a repeated card icon and not text/UI/watermark.
- If present, create visual_entity_map entity_role=hero_visual and semantic_png_plan task with extract_from_source.
- Composer will place hero_visual near header; do not turn it into a card.
...
- replacement_review: только entity_id + issue + decision + replacement для реально изменённых сущностей.
- Не возвращай medical_editorial_audit и image_composition_prompt: их заменяют smart_blocks/footer/design_blueprint.
...
Проанализируй медицинскую инфографику и верни compact v43.4 JSON.
...
Структура ответа:
{
  "schema_version": "v43.4-hero-preservation",
...
- Сохраняй hero_visual, если он объясняет тему всей инфографики.
...
- Не дублируй длинные объяснения и не возвращай служебные резюме.
