...
def _send_semantic_png_files(chat_id: int | str, asset_id: int) -> None:
    try:
        ...
        if not files:
            ...
            return
        send_message(chat_id, f"📦 Отправляю Semantic PNG исходника #{asset_id}: <b>{len(files)}</b> файл(ов).")
        for path in files[:20]:
            send_document(chat_id, str(path), caption=f"Semantic PNG #{asset_id}: <code>{_safe(path.name)}</code>")
        if len(files) > 20:
            send_message(chat_id, f"Показаны первые 20 файлов из {len(files)}.")
    except Exception as exc:
...
def _handle_command(chat_id: int | str, text: str, db: Session) -> None:
    if not text or text in {"/start", "/help"}:
        send_message(
            ...
            "/get_semantic_png 23 — скачать Semantic PNG\n"
    ...
    if text.startswith("/get_semantic_png"):
        ...
        if not asset_id:
            ...
            return
        _send_semantic_png_files(chat_id, asset_id)
        return
