...
def _run_full_pipeline_from_asset(chat_id: int | str, asset_id: int, db: Session) -> None:
    send_message(
        chat_id,
        f"🧠 Запускаю новую полную реконструкцию v40 для исходника #{asset_id}.\n\n"
        "Это автоматический pipeline: анализ → компоненты PNG → QA → layout → render → polish → final QA → пост.",
    )
...
def _handle_callback(callback_query: dict, db: Session) -> dict:
    ...
    try:
        if action == "approve":
            ...
            _clear_pending(chat_id)
            _run_full_pipeline_from_asset(chat_id, post_id, db)
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if text.startswith("/run_full_reconstruction") or text.startswith("/reconstruct_asset"):
        ...
        if not asset_id:
            ...
            return {"ok": True}
        try:
            _run_full_pipeline_from_asset(chat_id, asset_id, db)
        except Exception as exc:
            send_message(chat_id, _safe(str(exc)))
        return {"ok": True}
