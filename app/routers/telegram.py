...
def _handle_pending_asset(chat_id: int | str, message: dict, db: Session) -> bool:
    ...
    try:
        ...
        _clear_pending(chat_id)
        send_message(chat_id, "✅ Контент-исходник, паттерн и контекст сохранены.")
        send_message(chat_id, format_asset_card(asset))
        send_message(chat_id, format_pattern_card(pattern), reply_markup=_pattern_keyboard(pattern.id))
        return True
    except Exception as exc:
        _clear_pending(chat_id)
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if text == "/start" or text == "/help":
        send_message(
            ...
            "/analyze_asset — анализ контента + паттерна + контекста\n"
    ...
    if text == "/analyze_asset":
        ...
        send_message(
            ...
            "• текст поста;\n"
            "• ссылку с пояснением.\n\n"
            "Я сохраню 3 слоя: исходный контент, паттерн внимания и контекст.\n"
    ...
    if text == "/assets":
        ...
        for item in items:
            send_message(chat_id, format_asset_card(item))
        return {"ok": True}
