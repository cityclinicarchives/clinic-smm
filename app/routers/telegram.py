...
def _send_analysis_file(chat_id: int | str, asset_id: int) -> None:
    ...
    if not files:
        send_message(
            chat_id,
            f"❌ JSON анализа для исходника #{asset_id} не найден.\n"
            f"Ожидаемый путь: <code>storage/analysis/asset-{asset_id}-state-*-semantic-analysis.json</code>\n\n"
...
def _send_analysis_list(chat_id: int | str, limit: int = 20) -> None:
    ...
    if not files:
        send_message(chat_id, "JSON-файлов анализа пока нет в <code>storage/analysis</code>.")
        return
    lines = ["📁 <b>JSON-файлы v41-анализа:</b>"]
...
def _handle_command(chat_id: int | str, text: str, db: Session) -> None:
    if not text or text in {"/start", "/help"}:
        send_message(
            ...
            "Для инфографик сейчас доступен тест этапа 1 новой архитектуры: 🧠 Анализировать инфографику (v41).",
        )
        return
