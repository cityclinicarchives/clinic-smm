...
def _post_card(post, include_text: bool = False) -> str:
    ...
    if include_text:
        ...
        lines.append(f"/publish {post.id} — опубликовать в тестовую группу")
    return "\n".join(lines)
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if text.startswith("/generate"):
        ...
        send_message(chat_id, "Генерирую пост. Это может занять немного времени.")
    ...
    if text.startswith("/approve"):
        ...
        try:
            ...
            send_message(chat_id, f"Пост #{post.id} одобрен. Теперь его можно отправить: /publish {post.id}")
        except Exception as exc:
    ...
    if text.startswith("/rewrite"):
        ...
        send_message(chat_id, "Редактирую пост через ИИ. Это может занять немного времени.")
        try:
