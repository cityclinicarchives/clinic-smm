...
from app.services.full_pipeline_orchestrator import run_full_reconstruction_pipeline, FullPipelineError
...
RUNNING_FULL_PIPELINES: set[int] = set()
RUNNING_FULL_PIPELINES_LOCK = threading.Lock()
...
def _asset_keyboard(asset_id: int) -> dict:
    return {
        "inline_keyboard": [
            [
                {"text": "🧠 Реконструировать v40", "callback_data": f"asset_run_full:{asset_id}"},
            ]
        ]
    }
...
def _asset_is_infographic(asset) -> bool:
    """Best-effort classifier for Telegram UX.

    The strict classification is done inside the v40 master reconstruction call.
    Here we only decide whether to show the user the reconstruction button.
    """
    haystack = "\n".join([
        str(getattr(asset, "media_type", "") or ""),
        str(getattr(asset, "content_summary", "") or ""),
        str(getattr(asset, "analysis", "") or ""),
        str(getattr(asset, "caption", "") or ""),
        str(getattr(asset, "text_content", "") or ""),
    ]).lower()
    if any(word in haystack for word in ["инфограф", "infographic", "схема", "таблица", "чек-лист", "памятка"]):
        return True
    # Most uploaded pictures in /analyze_asset are visual assets suitable for reconstruction;
    # the v40 pipeline will do the real type detection and can still reject/repair internally.
    return (getattr(asset, "media_type", None) in {"photo", "document"})


def _send_asset_after_analysis(chat_id: int | str, asset) -> None:
    if _asset_is_infographic(asset):
        send_message(
            chat_id,
            format_asset_card(asset) + "\n\n<b>Тип:</b> похоже на инфографику/визуальный материал. Можно запускать полную v40-реконструкцию.",
            reply_markup=_asset_keyboard(asset.id),
        )
    else:
        send_message(
            chat_id,
            format_asset_card(asset) + "\n\nМатериал сохранен. Если это инфографика, можно всё равно запустить реконструкцию командой: /run_full_reconstruction " + str(asset.id),
        )


def _format_pipeline_steps(steps: list[dict], limit: int = 12) -> str:
    if not steps:
        return "Шаги pipeline не вернулись."
    lines = ["<b>Этапы v40 pipeline:</b>"]
    for step in steps[:limit]:
        name = _safe(str(step.get("step") or "stage"))
        status = _safe(str(step.get("status") or "?"))
        issues = step.get("issues") or []
        suffix = ""
        if issues:
            suffix = " — " + _safe("; ".join(map(str, issues[:2])))
        lines.append(f"• {name}: <b>{status}</b>{suffix}")
    if len(steps) > limit:
        lines.append(f"...и еще {len(steps) - limit} этапов")
    return "\n".join(lines)


def _asset_has_running_pipeline(db: Session, asset_id: int) -> int | None:
    """Return an in-progress ProjectState id for this asset, if any."""
    stages_done = {"completed", "failed"}
    try:
        state = (
            db.query(ProjectState)
            .filter(ProjectState.asset_id == asset_id)
            .order_by(ProjectState.id.desc())
            .first()
        )
        if state and str(state.pipeline_stage or "") not in stages_done:
            return int(state.id)
    except Exception:
        return None
    return None
...
def _run_full_pipeline_worker(chat_id: int | str, asset_id: int) -> None:
    db = SessionLocal()
    try:
        ...
        state_id = result.get("project_state_id")
        final_image_path = result.get("final_image_path")
        post_id = result.get("post_id")
        send_message(chat_id, f"✅ v40 pipeline завершен. Статус: <b>{_safe(status)}</b>. ProjectState: #{state_id}")
        send_message(chat_id, _format_pipeline_steps(result.get("steps") or []))
        if final_image_path:
            send_photo(chat_id, final_image_path, caption=f"Финальная инфографика v40 для исходника #{asset_id}")
        if post_id:
            post = get_post_or_raise(db, int(post_id))
            _send_post(chat_id, post, include_text=True)
        elif result.get("post_text"):
            send_message(chat_id, _safe(_shorten(result.get("post_text") or "")))
    except Exception as exc:
        message = str(exc)
        send_message(
            chat_id,
            "❌ v40 pipeline остановлен.\n\n"
            f"<b>Причина:</b> {_safe(message)}\n\n"
            "Если это validation_issues, пришлите сюда полный текст — теперь ошибка выводится подробно.",
        )
    finally:
        db.close()
        with RUNNING_FULL_PIPELINES_LOCK:
            RUNNING_FULL_PIPELINES.discard(asset_id)


def _start_full_pipeline_from_asset(chat_id: int | str, asset_id: int, db: Session) -> None:
    """Acknowledge Telegram instantly and run the slow v40 pipeline in background."""
    with RUNNING_FULL_PIPELINES_LOCK:
        if asset_id in RUNNING_FULL_PIPELINES:
            send_message(chat_id, f"⏳ Реконструкция v40 для исходника #{asset_id} уже выполняется. Дождитесь завершения.")
            return
        running_state_id = _asset_has_running_pipeline(db, asset_id)
        if running_state_id:
            send_message(chat_id, f"⏳ Для исходника #{asset_id} уже есть незавершенный ProjectState #{running_state_id}. Дождитесь завершения или проверьте состояние.")
            return
        RUNNING_FULL_PIPELINES.add(asset_id)

    send_message(
        chat_id,
        f"🧠 Принято. Запускаю новую полную реконструкцию v40 для исходника #{asset_id} в фоне.\n\n"
        "Pipeline: анализ → компоненты PNG → QA → layout → render → polish → final QA → пост.\n"
        "Я пришлю результат отдельным сообщением.",
    )
    thread = threading.Thread(target=_run_full_pipeline_worker, args=(chat_id, asset_id), daemon=True)
    thread.start()
...
def _handle_pending_asset(chat_id: int | str, message: dict, db: Session) -> bool:
    ...
    try:
        ...
        _clear_pending(chat_id)
        send_message(chat_id, "✅ Контент-исходник сохранен. Если это инфографика/визуальный материал, запускайте новую полную v40-реконструкцию кнопкой ниже.")
        _send_asset_after_analysis(chat_id, asset)
        return True
    except Exception as exc:
        _clear_pending(chat_id)
...
def _handle_callback(callback_query: dict, db: Session) -> dict:
    ...
    try:
        if action == "approve":
        ...
        elif action in {"asset_run_full", "asset_reconstruct"}:
            _clear_pending(chat_id)
            _start_full_pipeline_from_asset(chat_id, post_id, db)

        elif action in {"reconstruction_create", "reconstruction_create_full", "reconstruction_create_full_confirm"}:
            _clear_pending(chat_id)
            send_message(
                chat_id,
                "⚠️ Это кнопка старой системы реконструкции. Старый запуск отключен.\n\n"
                "Для новой v40-системы откройте исходник в /assets и нажмите «🧠 Реконструировать v40» "
                "или используйте команду: /run_full_reconstruction ID_исходника",
            )
            ...
            _clear_pending(chat_id)
            send_message(chat_id, "Старый режим повторной реконструкции отключен. Используйте /run_full_reconstruction ID_исходника.")
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if text == "/assets":
        ...
        for item in items:
            _send_asset_after_analysis(chat_id, item)
        return {"ok": True}
    ...
    if text.startswith("/run_full_reconstruction") or text.startswith("/reconstruct_asset"):
        command = "/run_full_reconstruction" if text.startswith("/run_full_reconstruction") else "/reconstruct_asset"
        asset_id, extra_instruction = _parse_id_and_payload(text, command)
        if not asset_id:
            send_message(chat_id, f"Укажите ID исходника. Например: {command} 1")
            return {"ok": True}
        try:
            _start_full_pipeline_from_asset(chat_id, asset_id, db)
        except Exception as exc:
            send_message(chat_id, _safe(str(exc)))
        return {"ok": True}
    ...
    if text.startswith("/create_full_from_reconstruction") or text.startswith("/create_from_reconstruction"):
        send_message(
            chat_id,
            "Старые команды реконструкции отключены. Используйте новую команду: /run_full_reconstruction ID_исходника",
        )
        return {"ok": True}
