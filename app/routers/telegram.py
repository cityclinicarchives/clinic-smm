...
from app.services.semantic_assets import generate_semantic_pngs, compose_reconstruction, list_semantic_png_paths, delete_semantic_png_artifacts
...
from app.services.content_intake import (
    build_intake_input_from_telegram_message,
    create_intake_item,
    export_intake_item_zip,
    format_intake_card,
    get_intake_item,
    list_intake_files,
    list_intake_items,
...
RUNNING_SEMANTIC_PNG_DOWNLOADS: set[str] = set()
...
def _intake_keyboard(item_id: int) -> dict:
    return {
        "inline_keyboard": [
            [
                {"text": "📦 Скачать исходник ZIP", "callback_data": f"content_export:{item_id}"},
            ],
            [
                {"text": "🧬 Адаптировать под соцсети", "callback_data": f"content_adapt:{item_id}"},
            ],
        ]
    }


def _send_intake_item(chat_id: int | str, db: Session, item_id: int) -> None:
    item = get_intake_item(db, item_id)
    if not item:
        send_message(chat_id, f"❌ Контент #{item_id} не найден.")
        return
    files = list_intake_files(db, item_id)
    send_message(chat_id, format_intake_card(item, files), reply_markup=_intake_keyboard(item.id))


def _handle_pending_new_content(chat_id: int | str, message: dict, db: Session) -> bool:
    pending = PENDING_ACTIONS.get(str(chat_id))
    if not pending or pending.get("mode") != "new_content":
        return False
    try:
        data = build_intake_input_from_telegram_message(message)
        item = create_intake_item(db, data, rubric=pending.get("rubric") or "expert")
        _clear_pending(chat_id)
        send_message(
            chat_id,
            "✅ Материал загружен в базу.\n\n" + format_intake_card(item, list_intake_files(db, item.id)),
            reply_markup=_intake_keyboard(item.id),
        )
    except Exception as exc:
        _clear_pending(chat_id)
        send_message(chat_id, f"❌ Не удалось загрузить контент: {_safe(str(exc))}")
    return True
...
def _generate_semantic_png_background(chat_id: int | str, asset_id: int) -> None:
    try:
        result = generate_semantic_pngs(asset_id)
        # v42.3 returns detailed stats; older local versions returned 3 values.
        if isinstance(result, tuple) and len(result) == 4:
            done, skipped, cost_summary, stats = result
        else:
            done, skipped, cost_summary = result  # type: ignore[misc]
            stats = {"extracted": len(done), "generated_ai": 0, "reused": len(skipped), "fallback": 0, "failed": 0, "failed_items": []}

        failed_items = stats.get("failed_items") or []
        ok_title = "✅ <b>Semantic PNG готовы</b>" if not failed_items else "⚠️ <b>Semantic PNG готовы частично</b>"
        lines = [
            ok_title,
            f"Исходник: <code>#{asset_id}</code>",
            f"Извлечено из исходника: <b>{int(stats.get('extracted', 0))}</b>",
            f"Сгенерировано AI: <b>{int(stats.get('generated_ai', 0))}</b>",
            f"Взято из хранилища/уже было: <b>{int(stats.get('reused', len(skipped)))}</b>",
            f"Fallback: <b>{int(stats.get('fallback', 0))}</b>",
            f"Ошибок генерации: <b>{int(stats.get('failed', 0))}</b>",
        ]
        groups = stats.get("style_lock_groups") or []
        shared_groups = [g for g in groups if isinstance(g, dict) and str(g.get("template_mode") or "").lower() == "shared"]
        if shared_groups:
            lines.append(f"Style lock groups: <b>{len(shared_groups)}</b>")
            for group in shared_groups[:3]:
                ref = _safe(str(group.get("reference_png_id") or group.get("reference_entity_id") or ""))
                sim = _safe(str(group.get("similarity") or ""))
                lines.append(f"• <code>{_safe(str(group.get('group_id') or 'group'))}</code>: ref <code>{ref}</code>, similarity <code>{sim}</code>")
        if stats.get("image_quality"):
            lines.append(f"AI image quality: <code>{_safe(str(stats.get('image_quality')))}</code>")
        if stats.get("image_input_fidelity"):
            lines.append(f"Reference image fidelity: <code>{_safe(str(stats.get('image_input_fidelity')))}</code>")
        if int(stats.get("reference_image_used", 0)) or int(stats.get("reference_image_failed", 0)):
            lines.append(f"Reference image used: <b>{int(stats.get('reference_image_used', 0))}</b>; failed: <b>{int(stats.get('reference_image_failed', 0))}</b>")
            for item in (stats.get("reference_image_items") or [])[:5]:
                lines.append(
                    f"• <code>{_safe(str(item.get('png_id') or 'png'))}</code> via ref <code>{_safe(str(item.get('reference_png_id') or ''))}</code>"
                )
        cost_text = format_cost_summary(cost_summary)
        if cost_text:
            lines.append(cost_text)
        if failed_items:
            lines.append("\n<b>Ошибки AI generation:</b>")
            for item in failed_items[:8]:
                png_id = _safe(str(item.get("png_id") or "png"))
                code = _safe(str(item.get("code") or "unknown_error"))
                msg = _safe(str(item.get("message") or ""))
                if len(msg) > 180:
                    msg = msg[:180] + "…"
                lines.append(f"• <code>{png_id}</code>: AI generation failed — <code>{code}</code>{(': ' + msg) if msg else ''}")
        lines.append(f"\nТеперь можно выполнить: <code>/compose_reconstruction {asset_id}</code>")
        send_message(chat_id, "\n".join(lines))
    except Exception as exc:
...
def _delete_semantic_png_cache_and_report(chat_id: int | str, asset_id: int, target: str | None) -> None:
    try:
        stats = delete_semantic_png_artifacts(asset_id, target)
        deleted_files = stats.get("deleted_files") or []
        target_label = stats.get("target") or "all"
        lines = [
            "🗑 <b>Semantic PNG удалены из хранилища</b>",
            f"Исходник: <code>#{asset_id}</code>",
            f"Цель: <code>{_safe(str(target_label))}</code>",
            f"Найдено файлов/записей: <b>{int(stats.get('matched', 0))}</b>",
            f"Удалено локально: <b>{int(stats.get('local_deleted', 0))}</b>",
            f"Удалено из R2: <b>{int(stats.get('remote_deleted', 0))}</b>",
            f"Удалено записей PostgreSQL: <b>{int(stats.get('db_deleted', 0))}</b>",
        ]
        if stats.get("manifest_deleted"):
            lines.append(f"Удалено manifest: <b>{int(stats.get('manifest_deleted', 0))}</b>")
        if deleted_files:
            preview = ", ".join(_safe(str(x)) for x in deleted_files[:12])
            if len(deleted_files) > 12:
                preview += f", … еще {len(deleted_files) - 12}"
            lines.append(f"Файлы: <code>{preview}</code>")
        if stats.get("errors"):
            lines.append("\n⚠️ <b>Предупреждения:</b>")
            for err in (stats.get("errors") or [])[:6]:
                lines.append(f"• <code>{_safe(str(err))}</code>")
        lines.append(f"\nДля пересоздания выполните: <code>/generate_semantic_png {asset_id}</code>")
        send_message(chat_id, "\n".join(lines))
    except Exception as exc:
        send_message(chat_id, f"❌ Не удалось удалить Semantic PNG #{asset_id}: {_safe(str(exc))}")

def _make_semantic_png_zip(asset_id: int, files: list[Path]) -> Path:
    """Create one ZIP archive with generated Semantic PNG files.

    Sending a single ZIP is intentionally safer than sending 10-20 Telegram
    documents directly from the webhook handler. Long synchronous sends can make
    Telegram retry the same update, which looks like an endless PNG loop.
    """
    export_dir = Path("storage/semantic_png_exports")
    export_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
    zip_path = export_dir / f"semantic-png-asset-{asset_id}-{ts}.zip"
...
def _send_semantic_png_zip_background(chat_id: int | str, asset_id: int, job_key: str) -> None:
    try:
        files = list_semantic_png_paths(asset_id)
        if not files:
            send_message(chat_id, f"❌ Semantic PNG для исходника #{asset_id} не найдены. Сначала выполните: <code>/generate_semantic_png {asset_id}</code>")
            return
        zip_path = _make_semantic_png_zip(asset_id, files)
        send_document(
            chat_id,
            str(zip_path),
            caption=(
                f"📦 Semantic PNG исходника #{asset_id}: <b>{len(files)}</b> файл(ов) в ZIP.\n"
                f"<code>{_safe(zip_path.name)}</code>"
            ),
        )
    except Exception as exc:
        send_message(chat_id, f"❌ Не удалось отправить Semantic PNG #{asset_id}: {_safe(str(exc))}")
    finally:
        RUNNING_SEMANTIC_PNG_DOWNLOADS.discard(job_key)


def _start_semantic_png_download(chat_id: int | str, asset_id: int) -> None:
    job_key = f"{chat_id}:{asset_id}"
    if job_key in RUNNING_SEMANTIC_PNG_DOWNLOADS:
        send_message(chat_id, f"⏳ Отправка Semantic PNG для исходника #{asset_id} уже выполняется. Повторную отправку не запускаю.")
        return
    RUNNING_SEMANTIC_PNG_DOWNLOADS.add(job_key)
    send_message(chat_id, f"📦 Готовлю ZIP с Semantic PNG для исходника #{asset_id}.")
    thread = threading.Thread(target=_send_semantic_png_zip_background, args=(chat_id, asset_id, job_key), daemon=True)
    thread.start()
...
async def telegram_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if _handle_pending_new_content(chat_id, message, db):
        return {"ok": True}
...
def _handle_command(chat_id: int | str, text: str, db: Session) -> None:
    if not text or text in {"/start", "/help"}:
        send_message(
            ...
            "/new_post — загрузить готовый контент в базу\n"
            "/content — список загруженного контента\n"
            "/content_12 — открыть карточку контента\n"
            "/export_content 12 — скачать исходник ZIP\n"
            ...
            "/get_semantic_png 23 — скачать Semantic PNG одним ZIP\n"
    ...
    if text == "/new_post":
        _set_pending_mode(chat_id, "new_content", rubric="expert")
        send_message(
            chat_id,
            "📥 Отправьте готовый материал для загрузки в базу: текст, картинку с caption, видео или документ.\n\n"
            "Пока это v44.1: сохраняем исходник и даём скачать ZIP для ручной публикации."
        )
        return
    if text == "/content":
        items = list_intake_items(db, limit=10)
        if not items:
            send_message(chat_id, "Контент пока не загружен. Используйте /new_post")
            return
        for item in items:
            send_message(chat_id, format_intake_card(item, list_intake_files(db, item.id)), reply_markup=_intake_keyboard(item.id))
        return
    if text.startswith("/content_"):
        try:
            item_id = int(text.split("_", 1)[1].strip())
        except Exception:
            send_message(chat_id, "Укажите ID, например: <code>/content_12</code>")
            return
        _send_intake_item(chat_id, db, item_id)
        return
    if text.startswith("/export_content"):
        item_id, _ = _parse_id_and_payload(text, "/export_content")
        if not item_id:
            send_message(chat_id, "Укажите ID. Например: <code>/export_content 12</code>")
            return
        try:
            path = export_intake_item_zip(db, item_id)
            send_document(chat_id, str(path), caption=f"📦 ZIP исходного контента #{item_id}")
        except Exception as exc:
            send_message(chat_id, f"❌ Не удалось собрать ZIP: {_safe(str(exc))}")
        return
    ...
    if text.startswith("/generate_semantic_png"):
        ...
        return
    if text.startswith("/get_semantic_png"):
        asset_id, _ = _parse_id_and_payload(text, "/get_semantic_png")
        if not asset_id:
            send_message(chat_id, "Укажите ID исходника. Например: <code>/get_semantic_png 23</code>")
            return
        _start_semantic_png_download(chat_id, asset_id)
        return

    if text.startswith("/delete_semantic_png"):
        asset_id, payload = _parse_id_and_payload(text, "/delete_semantic_png")
        if not asset_id:
            send_message(chat_id, "Укажите ID исходника. Например: <code>/delete_semantic_png 23</code> или <code>/delete_semantic_png 23 png_002</code>")
            return
        target = (payload or "all").strip()
        _delete_semantic_png_cache_and_report(chat_id, asset_id, target)
        return
