...
def _clamp_png_size(size: Dict[str, Any], operation: str = "") -> Dict[str, int]:
    """Normalize LLM-provided PNG sizes before Pydantic validation.

    The analytical model can return tiny label blocks like 900x48. Our
    ImageTask schema requires at least 64px, and for text PNG blocks the
    renderer needs a taller safe area. We must repair the contract instead of
    stopping the pipeline.
    """
    try:
        w = int(size.get("w") or size.get("width") or 0)
        h = int(size.get("h") or size.get("height") or 0)
    except Exception:
        w, h = 0, 0
    if operation == "generate_text_png_block":
        return {"w": max(w or TEXT_SIZE["w"], 300), "h": max(h or TEXT_SIZE["h"], 120)}
    if operation == "generate_background":
        return {"w": max(w or BACKGROUND_SIZE["w"], 64), "h": max(h or BACKGROUND_SIZE["h"], 64)}
    return {"w": max(w or DEFAULT_SIZE["w"], 64), "h": max(h or DEFAULT_SIZE["h"], 64)}
...
def _size_for_unit(unit: Dict[str, Any], operation: str) -> Dict[str, int]:
    ...
    if isinstance(raw, dict) and (raw.get("w") or raw.get("width")) and (raw.get("h") or raw.get("height")):
        return _clamp_png_size(raw, operation)
    if operation == "generate_text_png_block":
...
def _normalize_existing_task(raw: Dict[str, Any], payload: ProjectStatePayload) -> Tuple[ImageTask | None, List[str]]:
    ...
    operation_for_size = str(task.get("operation") or "")
    if "output_png_size" not in task or not isinstance(task.get("output_png_size"), dict):
        task["output_png_size"] = _size_for_unit(unit, operation_for_size)
    else:
        task["output_png_size"] = _clamp_png_size(task.get("output_png_size") or {}, operation_for_size)
...
def _generate_missing_tasks(payload: ProjectStatePayload, existing: Iterable[ImageTask]) -> List[ImageTask]:
    existing_units = {task.final_unit_id for task in existing}
    tasks: List[ImageTask] = []
    for unit in payload.final_units:
        if not isinstance(unit, dict):
            continue
        fid = _unit_id(unit)
        if not fid or fid in existing_units:
            continue
        component_items = _component_map_for_unit(payload, fid)
        operation = _operation_for_unit(unit, component_items)
