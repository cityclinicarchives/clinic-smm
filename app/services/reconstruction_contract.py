...
def _content_text_for_final_units(spec: dict[str, Any]) -> str:
    ...
    for b in blocks:
        keep.append({
            ...
            "source_policy": b.get("source_policy"),
            "old_element": b.get("old_element"),
            "new_element": b.get("new_element"),
            "replacement_prompt": b.get("replacement_prompt"),
        })
    return _text_blob({
        "title": spec.get("title"),
        "structure_blocks": keep,
        "visual_must_include": (spec.get("visual") or {}).get("must_include"),
        "visual_must_avoid": (spec.get("visual") or {}).get("must_avoid"),
    })
