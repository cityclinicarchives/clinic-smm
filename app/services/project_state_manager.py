    ...
    *,
    ...
    state = ProjectState(
        ...
        stage_history_json=_to_json([
            {"stage": pipeline_stage, "event": "created", "state_version": 1}
        ]),
    )
    db.add(state)
    db.commit()
...
def get_payload(state: ProjectState) -> ProjectStatePayload:
    return ProjectStatePayload.model_validate(_from_json(state.payload_json, {}))


def get_continuation_package(state: ProjectState) -> ContinuationPackage:
    return ContinuationPackage.model_validate(_from_json(state.continuation_package_json, {}))
...
def build_ai_context_from_state(state: ProjectState) -> Dict[str, Any]:
    ...
    return {
        ...
        "visual_entity_map": payload.visual_entity_map,
        "semantic_png_plan": payload.semantic_png_plan,
        "design_blueprint": payload.design_blueprint,
        "post": payload.post,
