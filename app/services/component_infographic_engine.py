...
from app.services.ai_crop_planner import plan_crops_with_ai, validate_crop_plan, repair_crop_plan_with_ai
from app.services.visual_decomposition_engine import build_visual_decomposition, validate_component_map, crop_plan_from_component_map
from app.services.source_unit_decision_engine import decide_source_units_with_ai, apply_source_unit_decisions, validate_source_unit_decisions
from app.services.semantic_layout_engine import (
    build_semantic_layout_map,
    validate_semantic_layout,
    layout_context_for_prompt,
    enforce_card_boundaries_on_component_map,
)
...
def _card_lookup_keys(block: dict[str, Any], idx: int | None = None) -> list[str]:
    keys = []
    for k in (block.get("id"), block.get("unit_id"), block.get("title"), block.get("visual_element"), block.get("new_element")):
        if k:
            keys.append(str(k).strip().lower())
    if idx is not None:
        keys.append(f"unit_{idx}".lower())
    return keys


def _crop_plan_item_for_block(crop_plan: dict[str, Any] | None, block: dict[str, Any], idx: int | None = None) -> dict[str, Any] | None:
    if not isinstance(crop_plan, dict):
        return None
    items = crop_plan.get("crop_plan")
    if not isinstance(items, list):
        return None
    keys = set(_card_lookup_keys(block, idx))
    for item in items:
        if not isinstance(item, dict):
            continue
        item_keys = set()
        for k in (item.get("unit_id"), item.get("id"), item.get("title")):
            if k:
                item_keys.add(str(k).strip().lower())
        if keys & item_keys:
            return item
    return None
...
def _apply_source_unit_decision_engine(source: Image.Image, cards: list[dict[str, Any]], spec: dict[str, Any]) -> tuple[list[dict[str, Any]], list[str]]:
    """v38: build final units from medical/regional relevance before crop planning.

    The reconstruction blueprint may contain source units mechanically copied from
    the original. This stage asks AI to decide keep/merge/replace/remove and
    returns final cards only. If the AI step fails, the original cards are kept
    and diagnostic issues are returned.
    """
    try:
        client = _get_client()
        decisions = decide_source_units_with_ai(
            client=client,
            model=settings.openai_model,
            source_image=source,
            cards=cards,
            contract_summary_text=contract_summary(spec),
        )
        spec["_source_unit_decisions"] = decisions
        issues = validate_source_unit_decisions(decisions)
        new_cards, apply_issues = apply_source_unit_decisions(cards, decisions)
        issues.extend(apply_issues)
        return new_cards, issues
    except Exception as exc:
        return cards, [f"source_unit_decision_failed:{exc}"]


def _build_semantic_layout(source: Image.Image, cards: list[dict[str, Any]], spec: dict[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    """Build a layout-first semantic card map before component decomposition.

    This catches cases where object-based AI splits one semantic card into
    multiple cards or misses the boundary of the logical card.
    """
    try:
        client = _get_client()
        layout = build_semantic_layout_map(
            client=client,
            model=settings.openai_model,
            source_image=source,
            cards=cards,
        ...
        issues = validate_semantic_layout(layout, cards)
        spec["_semantic_layout"] = layout
        return layout, issues
    except Exception as exc:
        return None, [f"semantic_layout_failed:{exc}"]


def _build_ai_crop_plan(source: Image.Image, cards: list[dict[str, Any]], spec: dict[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    """Build crop plan using v36 Visual Decomposition first, then legacy crop planner.

    v36 is structure-first: AI decomposes the infographic into semantic/design
    components (visual objects, labels, backgrounds, UI). The crop plan is then
    derived from preserved visual components. The older bbox planner is only a
    ...
    try:
        ...
        return None, [f"openai_client_failed:{exc}"]

    decomposition_issues: list[str] = []
    semantic_layout, semantic_layout_issues = _build_semantic_layout(source, cards, spec)
    try:
        layout_context = layout_context_for_prompt(semantic_layout or {})
        component_map = build_visual_decomposition(
            client=client,
            model=settings.openai_model,
            source_image=source,
            cards=cards,
            contract_summary_text=contract_summary(spec) + layout_context,
        )
        if semantic_layout:
            component_map = enforce_card_boundaries_on_component_map(component_map, semantic_layout)
        spec["_visual_decomposition"] = component_map
        decomposition_issues = semantic_layout_issues + validate_component_map(component_map, cards)
        plan = crop_plan_from_component_map(component_map, cards)
        issues = validate_crop_plan(plan, cards)
        # Decomposition issues are useful but not always fatal: generated_new is
        # acceptable for elements that cannot be safely extracted.
        hard_decomp = [x for x in decomposition_issues if ("missing_unit" in x or "semantic_layout_too_many_cards" in x or "possible_bad_split" in x)]
        if not issues and not hard_decomp:
            return plan, []
        # One repair attempt through the older crop-plan repair prompt. It uses
        # the component-derived plan as the starting point.
        try:
            repaired = repair_crop_plan_with_ai(
                client=client,
                model=settings.openai_model,
                source_image=source,
                cards=cards,
                crop_plan=plan,
                issues=issues + hard_decomp,
                contract_summary_text=contract_summary(spec),
            )
            repaired_issues = validate_crop_plan(repaired, cards)
            if not repaired_issues:
                repaired["source"] = "visual_decomposition_engine_repaired"
                return repaired, []
            return repaired, repaired_issues + decomposition_issues
        except Exception:
            return plan, issues + decomposition_issues
    except Exception as exc:
        decomposition_issues.append(f"visual_decomposition_failed:{exc}")

    # Legacy fallback: AI Crop Planner. Kept for compatibility, but if it also
    # fails the caller should stop before assembly.
    try:
        plan = plan_crops_with_ai(
            client=client,
            model=settings.openai_model,
            source_image=source,
            cards=cards,
        ...
        if issues:
            try:
                ...
                repaired_issues = validate_crop_plan(repaired, cards)
                if not repaired_issues:
                    return repaired, decomposition_issues
                return repaired, decomposition_issues + repaired_issues
            except Exception:
                return plan, decomposition_issues + issues
        return plan, decomposition_issues
    except Exception as exc:
        return None, decomposition_issues + [f"crop_planner_failed:{exc}"]


def _raw_crop_from_plan_or_bbox(source: Image.Image, block: dict[str, Any], crop_plan_item: dict[str, Any] | None = None) -> tuple[Image.Image | None, str]:
    """Return raw crop/composite from AI Crop Planner, falling back to source_bbox.

    status tells the preview/final renderer how the crop was obtained.
    """
    if isinstance(crop_plan_item, dict):
        strategy = str(crop_plan_item.get("raw_crop_strategy") or "").lower()
        if strategy == "impossible_use_generate_new":
            return None, "generated_new_by_crop_plan"
        parts = crop_plan_item.get("crop_parts")
        if isinstance(parts, list) and parts:
            crops: list[tuple[str, Image.Image]] = []
            for part in parts:
                if not isinstance(part, dict):
                    continue
                bbox = _normalize_bbox(part.get("bbox"), source.width, source.height)
                if not bbox:
                    continue
                img = ImageOps.exif_transpose(source.crop(bbox).convert("RGB"))
                # light local trim to reduce labels/background even before AI clean
                img = _semantic_trim_visual_crop(img, block)
                crops.append((str(part.get("part") or "part"), img))
            if len(crops) == 1:
                return crops[0][1], "single_bbox_from_ai_crop_plan"
            if len(crops) > 1:
                # Create a clean contact-sheet reference that AI cleaner can combine.
                max_h = max(im.height for _, im in crops)
                total_w = sum(im.width for _, im in crops) + 28 * (len(crops) + 1)
                canvas = Image.new("RGB", (max(240, total_w), max(180, max_h + 56)), "#FFFFFF")
                x = 28
                for _, im in crops:
                    y = (canvas.height - im.height) // 2
                    canvas.paste(im, (x, y))
                    x += im.width + 28
                return canvas, "multi_part_bbox_from_ai_crop_plan"
    # Fallback: old bbox only as raw material, not as final clean component.
    ...
    if not bbox:
        return None, "no_bbox"
    crop = ImageOps.exif_transpose(source.crop(bbox).convert("RGB"))
    return crop, "fallback_blueprint_bbox"
...
def _crop_from_source(source: Image.Image, block: dict[str, Any], crop_plan_item: dict[str, Any] | None = None) -> Image.Image | None:
    # v35: source_bbox from reconstruction blueprint is not trusted directly.
    # A dedicated AI Crop Planner returns tight single/multi-part crop areas.
    # Raw crop is only a source material for AI-cleaning, never final visual.
    area = _bbox_area_ratio(block.get("source_bbox"))
    block_type = str(block.get("type") or "").lower()
    if not crop_plan_item and block_type in {"comparison_card", "card", "tile", "visual_card", "comparison_item"} and area is not None and area > 0.22:
        return None
    raw, _status = _raw_crop_from_plan_or_bbox(source, block, crop_plan_item=crop_plan_item)
    if raw is None:
        return None
    ...
    # local trim is cheap first-aid; AI clean is executed later in _prepare_visual
    if policy == "use_reference_and_clean" or "clean" in crop_mode or "object" in crop_mode:
        raw = _semantic_trim_visual_crop(raw, block)
    return raw
...
Keep from reference only the medically useful visual elements, preserving medical roles:
- PRIMARY medical object: the full bite/skin reaction/lesion/rash area. This must never be lost.
- SECONDARY context object: the insect/arthropod/context icon if present and relevant.
If primary and secondary overlap, keep both; do not replace either with text.
...
- realistic/clear primary bite/skin reaction plus secondary insect/context object in the same visual style as the source;
- for circular bite photos, preserve the whole circle/skin patch, not only the red center;
...
def _prepare_visual(source: Image.Image, block: dict[str, Any], style: str, target_w: int, target_h: int, crop_plan_item: dict[str, Any] | None = None) -> Image.Image:
    ...
    if policy in {"preserve_from_reference", "use_reference_and_clean"}:
        raw_crop = _crop_from_source(source, block, crop_plan_item=crop_plan_item)
        if raw_crop is not None and cleanup_mode:
            # v35: raw crop is never accepted as final if use_reference_and_clean is requested.
            # It must be transformed into a clean visual component by AI.
            visual = _clean_reference_crop_with_ai(raw_crop, block, style=style, size=max(target_w, target_h))
            if visual is None:
                # If cleanup failed, use generated replacement/placeholder instead of dirty raw crop.
...
def analyze_component_infographic_blueprint(
    ...
    cards, source_unit_issues = _apply_source_unit_decision_engine(source, cards, spec)
    crop_plan, crop_plan_issues = _build_ai_crop_plan(source, cards, spec)
    if crop_plan:
        spec["_crop_plan"] = crop_plan
    issues: list[str] = []
    issues.extend(source_unit_issues)
    issues.extend(crop_plan_issues)
    ...
    for idx, block in enumerate(cards, start=1):
        ...
        crop_size = None
        if policy in {"preserve_from_reference", "use_reference_and_clean"}:
            crop = _crop_from_source(source, block, crop_plan_item=_crop_plan_item_for_block(crop_plan, block, idx))
...
def generate_component_crop_preview_image(
    ...
    if source_unit_issues:
        report.setdefault("issues", []).extend(source_unit_issues)
    if crop_plan_issues:
        report.setdefault("issues", []).extend(crop_plan_issues)
        report.setdefault("critical_issues", []).extend([x for x in crop_plan_issues if "bad_bbox" in x or "missing" in x])
        report["ok"] = not report.get("critical_issues")
    ...
    for idx, block in enumerate(cards, start=1):
        ...
        crop = None
        item = _crop_plan_item_for_block(crop_plan, block, idx)
        if policy.lower() in {"preserve_from_reference", "use_reference_and_clean"}:
            # Show cleaned visual component, not raw rectangular crop.
            crop = _prepare_visual(source, block, style="clean medical component", target_w=tile_w - 70, target_h=145, crop_plan_item=item)
        if crop is not None:
            crop.thumbnail((tile_w - 36, 150), Image.LANCZOS)
            canvas.paste(crop, (x + (tile_w - crop.width)//2, y + 76))
            status_label = "clean crop OK" if policy.lower() == "use_reference_and_clean" else "crop OK"
            draw.text((x + 16, y + tile_h - 32), status_label, font=small_font, fill="#1D918E")
...
def _render_technical_draft(
    ...
    for idx, block in enumerate(cards):
        ...
        visual_y = visual_top
        visual = _prepare_visual(source, block, style, target_w=visual_w, target_h=visual_h, crop_plan_item=_crop_plan_item_for_block(spec.get("_crop_plan"), block, idx + 1))
...
def generate_crop_assembled_infographic_image(
    ...
    if crop_plan:
        spec["_crop_plan"] = crop_plan
    critical_source_unit_issues = [x for x in source_unit_issues if "failed" in x or "too_few" in x or "no_final_units" in x]
    if critical_source_unit_issues:
        raise ComponentInfographicError("Критические проблемы Source Unit Decision перед сборкой:\n" + "\n".join(f"- {x}" for x in critical_source_unit_issues[:12]))
    if crop_plan_issues and any(("bad_bbox" in x or "missing" in x or "no_parts" in x) for x in crop_plan_issues):
        raise ComponentInfographicError("Критические проблемы AI Crop Planner перед сборкой:\n" + "\n".join(f"- {x}" for x in crop_plan_issues[:12]))
    ...
    validation_issues.extend(source_unit_issues)
    validation_issues.extend(crop_plan_issues)
