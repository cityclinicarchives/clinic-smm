...
def _crop_from_source(source: Image.Image, block: dict[str, Any]) -> Image.Image | None:
    # v27 sanity check: an atomic visual card cannot crop the whole grid/collage.
    # If bbox is too large, force repair/generation instead of inserting the full source inside one card.
...
def analyze_component_infographic_blueprint(
    ...
    for idx, block in enumerate(cards, start=1):
        ...
        if policy in {"preserve_from_reference", "use_reference_and_clean"}:
            ...
            if crop is None:
                crop_ok = False
                crop_issue = f"card_{idx}_crop_failed_or_not_atomic"
...
def generate_crop_assembled_infographic_image(
    ...
    image_bytes = download_file_bytes(asset.media_file_id)  # type: ignore[arg-type]
    # v27: if the first blueprint is not atomic enough, repair it before rendering.
    ...
    if pre_issues:
        try:
            spec = repair_atomic_blueprint_with_ai(
                ...
                issues=pre_issues,
                extra_context="Repair before crop-and-assemble rendering",
            )
        except Exception:
            # Continue with original blueprint, but the prompt report will include issues.
            pass
    source = Image.open(BytesIO(image_bytes)).convert("RGB")
    source = ImageOps.exif_transpose(source)
    ...
    ai_ok, ai_layouts, ai_layout_issues = validate_ai_layout(cards, W, H)
    # Use AI card coordinates only when they are safe AND the infographic is not dense.
    # For dense infographics, deterministic v26 layout is more reliable and prevents cropping.
    if ai_ok and len(cards) <= 6:
    ...
    # auto_plan_layout may escalate canvas height. Use the real plan canvas.
    ...
    critical_validation_issues = _critical_issues(validation_issues)
    ...
    images_dir = Path(settings.generated_images_dir)
    images_dir.mkdir(parents=True, exist_ok=True)
    filename = f"v27-atomic-infographic-{reconstruction.id}-post-{post.id}-{_slugify(post.title)}.png"
    path = images_dir / filename
    canvas.save(path, format="PNG", optimize=True)

    prompt_report = "\n".join([
        "v27 Atomic Blueprint + v26 Strict Blueprint Validator + Multi-format Escalation + Final QA",
        ...
        "Правило v26: все blueprint-карточки и основные footer-блоки должны быть отрендерены; canvas расширяется, а контент не сжимается и не обрезается.",
    ])
