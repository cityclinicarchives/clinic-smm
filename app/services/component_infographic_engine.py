...
def generate_crop_assembled_infographic_image(
    ...
    filename = f"v26-blueprint-infographic-{reconstruction.id}-post-{post.id}-{_slugify(post.title)}.png"
    path = images_dir / filename
    canvas.save(path, format="PNG", optimize=True)

    prompt_report = "\n".join([
        "v26 Strict Blueprint Validator + Multi-format Escalation + Final QA",
