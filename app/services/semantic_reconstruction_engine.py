...
def normalize_semantic_plan(data: Dict[str, Any]) -> tuple[Dict[str, Any], List[str]]:
    ...
    for i, entity in enumerate(entities, start=1):
        ...
        if not isinstance(semantic_png, dict):
            ...
            visual_roles = {"comparison_item", "visual_explanation", "icon"}
...
def _important_review_item(item: Dict[str, Any]) -> bool:
    ...
    issue = str(item.get("issue") or item.get("initial_problem") or "").lower()
...
def _compact_audit(audit: Any) -> Dict[str, Any]:
    ...
    return {
        "risks": _compact_list(audit.get("risks"), 3, 120),
        "minimal_corrections": _compact_list(audit.get("minimal_corrections") or audit.get("corrections"), 3, 120),
        "footer_warnings": _compact_list(audit.get("footer_warnings") or audit.get("required_warnings"), 3, 160),
    }
...
def _compact_source_pattern(pattern: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "visual_strengths": _compact_list(pattern.get("visual_strengths"), 3, 75),
        "what_to_preserve": _compact_list(pattern.get("what_to_preserve"), 4, 75),
        "what_to_fix": _compact_list(pattern.get("what_to_fix"), 4, 75),
    }
...
def compact_semantic_payload(data: Dict[str, Any]) -> Dict[str, Any]:
    """
    v43.1-smart-blocks: minimal storage contract.
    ...
    for card in raw_cards:
        ...
        content_cards.append({
            ...
            "short_text": _compact_text(card.get("short_text"), 95),
        })
        layout_cards.append({
            "card_id": card.get("card_id"),
            "entity_id": card.get("entity_id"),
    ...
    return {
        ...
        "schema_version": data.get("schema_version") or "v43.1-smart-blocks",
        ...
        "image_composition_prompt": _compact_text(data.get("image_composition_prompt"), 500),
        ...
        "qa_checklist": _compact_list(data.get("qa_checklist"), 8, 100),
    }
