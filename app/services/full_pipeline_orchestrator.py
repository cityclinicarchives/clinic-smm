    ...
    *,
    ...
    try:
        ...
        if not getattr(task_plan, "ready", False):
            ...
            issues = getattr(task_plan, "validation_issues", None) or []
            details = "; ".join(map(str, issues[:12])) if issues else "no validation_issues returned"
            raise FullPipelineError(f"Image task preparation failed: {details}")
        # Even when ready=True, keep normalization warnings visible in steps.
        prepare_status = "ok_with_warnings" if getattr(task_plan, "validation_issues", None) else "ok"
        steps.append(_step("image_tasks_prepare", prepare_status, task_plan))
