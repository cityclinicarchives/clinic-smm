...
from app.services.component_infographic_engine import generate_component_infographic_image, has_component_reference
...
def reconstruct_asset_with_ai(db: Session, asset_id: int, instruction: str | None = None) -> ContentReconstruction:
    ...
    # v23: для инфографик с исходным изображением основной режим — component_reference_infographic.
    ...
    if has_reference_image(asset) and (
        ...
        or str(visual.get("renderer_mode") or "").lower() == "component_reference_infographic"
    ):
        visual["renderer_mode"] = "component_reference_infographic"
    ...
    if visual.get("renderer_mode") == "deterministic_infographic":
        visual["renderer_mode"] = "component_reference_infographic" if has_reference_image(asset) else "ai_image"
...
def create_post_from_reconstruction(db: Session, reconstruction_id: int, with_image: bool = True) -> ContentPost:
    ...
    if with_image:
        ...
        if mode == "component_reference_infographic" and has_component_reference(asset):
            try:
                image_path, image_prompt = generate_component_infographic_image(
