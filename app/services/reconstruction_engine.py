...
from app.services.reconstruction_contract import contract_summary
from app.services.component_infographic_engine import (
    generate_component_infographic_image,
    generate_crop_assembled_infographic_image,
    generate_component_crop_preview_image,
    analyze_component_infographic_blueprint,
...
def _render_template(template: str, values: dict[str, str]) -> str:
    """Render prompt templates safely.

    We intentionally do NOT use str.format() because our prompt templates contain
    JSON examples with curly braces. str.format() treats every JSON key as a
    placeholder and can crash with short errors like: * or x.
    """
    rendered = template
...
def reconstruct_asset_with_ai(db: Session, asset_id: int, instruction: str | None = None) -> ContentReconstruction:
    ...
    prompt = _render_template(prompt_choice.user_template, prompt_context)
    ...
    # v27: make infographic blueprint atomic before saving, not only before rendering.
    content_type = str(spec.get("content_type") or prompt_choice.asset_type or "").lower()
    kind = str(spec["structure"].get("kind") or "").lower()
    if has_reference_image(asset) and (content_type in {"infographic", "carousel"} or kind in {"infographic", "component_infographic", "carousel", "crop_and_assemble_infographic"}):
        spec = normalize_atomic_blocks(spec)
        issues = atomic_blueprint_issues(spec)
        if issues:
            try:
                image_bytes_for_repair = download_file_bytes(asset.media_file_id) if asset.media_file_id else None
                spec = repair_atomic_blueprint_with_ai(
                    client=client,
                    model=settings.openai_model,
                    spec=spec,
                    image_bytes=image_bytes_for_repair,
                    issues=issues,
                    extra_context="Repair during reconstruction before saving",
                )
                spec.setdefault("router", {})
                spec["router"].update({
                    "asset_type": prompt_choice.asset_type,
                    ...
                    "recommended_pipeline": prompt_choice.recommended_pipeline,
                    "reason": prompt_choice.reason,
                    "v27_repair_applied": True,
                    "v27_repair_initial_issues": issues,
                })
            except Exception as repair_exc:
                spec.setdefault("router", {})
                spec["router"].update({"v27_repair_failed": str(repair_exc), "v27_repair_initial_issues": issues})
...
def _generate_post_text_from_reconstruction_spec(spec: dict[str, Any], reconstruction: ContentReconstruction) -> str:
    """v32: write post from post_brief + final reconstruction contract, not generic topic."""
    fallback = _spec_text(spec, "post", "text") or reconstruction.post_text or reconstruction.analysis or ""
    post_brief = spec.get("post_brief") or {}
    medical_audit = spec.get("medical_audit") or {}
    structure = spec.get("structure") or {}
    try:
        client = _get_client()
        prompt = f"""
...
Напиши сопровождение к КОНКРЕТНОЙ реконструированной инфографике. Нельзя писать общий пост «на тему».
Пост должен использовать именно финальные блоки инфографики, post_brief и reconstruction_contract.

ИСТОЧНИКИ, ИЗ КОТОРЫХ МОЖНО БРАТЬ СМЫСЛ:
1. post_brief — главное ТЗ для поста;
2. structure.blocks — финальные блоки инфографики;
3. medical_audit — медицинские предупреждения, safe actions, prevention;
4. reconstruction_contract — обязательные элементы, forbidden elements и replacement rules.

ОБЯЗАТЕЛЬНО ВКЛЮЧИ:
- что показывает именно эта инфографика;
- зачем ее сохранять;
- ограничения: по картинке нельзя ставить точный диагноз;
- warning signs / когда обращаться к врачу;
- safe actions / что можно сделать сразу безопасно;
- prevention, если есть;
- мягкий CTA к консультации.

ОБЯЗАТЕЛЬНО ИЗБЕГАЙ:
- общей статьи без связи с инфографикой;
- обещания точной диагностики;
- новых медицинских фактов, которых нет в spec;
- упоминания forbidden_elements как актуальных финальных элементов;
- агрессивной рекламы и запугивания.

Стиль: спокойно, экспертно, понятно, для Telegram медицинской клиники в Москве.
Длина: 900–1600 знаков.

post_brief:
{json.dumps(post_brief, ensure_ascii=False)[:6000]}

medical_audit:
{json.dumps(medical_audit, ensure_ascii=False)[:6000]}

final structure.blocks:
{json.dumps(structure.get('blocks') or [], ensure_ascii=False)[:9000]}

Reconstruction contract:
{contract_summary(spec)}

Полный structured spec для справки:
{json.dumps(spec, ensure_ascii=False)[:16000]}

Верни только готовый текст поста без JSON и markdown-таблиц.
""".strip()
        response = client.responses.create(
            model=settings.openai_model,
            input=[
                {"role": "system", "content": "Ты пишешь медицинские SMM-посты строго по post_brief и финальной инфографике. Соблюдай медицинскую осторожность."},
...
def get_reconstruction_or_raise(db: Session, reconstruction_id: int) -> ContentReconstruction:
...
def reconstruction_needs_crop_preview(db: Session, reconstruction_id: int) -> bool:
    reconstruction = get_reconstruction_or_raise(db, reconstruction_id)
    ...
    return mode == "crop_and_assemble_infographic" and has_component_reference(asset)


def create_crop_preview_for_reconstruction(db: Session, reconstruction_id: int) -> tuple[str, str, list[str]]:
    reconstruction = get_reconstruction_or_raise(db, reconstruction_id)
    asset = db.query(ContentAsset).filter(ContentAsset.id == reconstruction.asset_id).first() if reconstruction.asset_id else None
    return generate_component_crop_preview_image(reconstruction=reconstruction, asset=asset)
...
def create_post_from_reconstruction(db: Session, reconstruction_id: int, with_image: bool = True) -> ContentPost:
    ...
    # v29: post is generated from the full reconstruction spec, not from a short topic.
    text = _generate_post_text_from_reconstruction_spec(spec, reconstruction)
    ...
    if with_image:
        ...
        if mode == "crop_and_assemble_infographic" and has_component_reference(asset):
            # v32: no unsafe fallback for complex infographic reconstruction.
            # If strict crop-and-assemble fails, stop and show the error instead of
            # silently switching to generic image generation/reference edit.
