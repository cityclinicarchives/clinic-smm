...
def choose_format(spec: dict[str, Any], content_cards_count: int, footer_blocks_count: int) -> dict[str, Any]:
    ...
    if content_cards_count <= 4 and total_blocks <= 7:
        ...
        chosen = "4:5"
    elif content_cards_count <= 9 and total_blocks <= 13:
        chosen = "3:4"
    else:
        chosen = "2:3"
...
def required_canvas_height(W: int, n_cards: int, n_footers: int, min_card_h: int = 340) -> int:
    margin = 34
    gap = 18
    header_h = 205
    ...
    footer_h = 0 if footer_rows == 0 else footer_rows * 270 + (footer_rows - 1) * gap + 40
