...
def _component_cleanup_and_trim(img: Image.Image) -> Image.Image:
    ...
    # 1) Detect robust foreground seeds. The largest component is always part of
    # the object. Additional large non-text components are also protected (for
    # example an insect body detached from a skin circle by transparent pixels).
    protected_ids: set[int] = {id(comps[0])}
    seed_area_threshold = max(650, int(total_area * 0.030))
    for c in comps[1:]:
        if c['area'] >= seed_area_threshold and not looks_like_text_component(c):
            protected_ids.add(id(c))

    # 2) Iteratively protect components that touch/are very close to protected
    # components. This preserves thin connected details, legs and antennae while
    # avoiding the previous overly-wide protection radius that preserved captions.
    changed = True
    close_touch = max(3, int(min(w, h) * 0.018))
    while changed:
        changed = False
        ...
        for c in comps:
            if id(c) in protected_ids:
                continue
            # Protect sizeable semantic parts and tiny details if they are really
            # close to a protected object. Do not protect far-away text lines.
            near = any(bbox_distance(pb, c['bbox']) <= close_touch for pb in protected_boxes)
            if near:
                protected_ids.add(id(c))
                changed = True
    ...
    # 6) Keep-main-object-cluster mode. After edge-aware background removal,
    # the safest generic rule is: keep only the main foreground object cluster
    # and remove everything detached from it.
    #
    # This is intentionally stricter than text suppression. It removes leftover
    # single letters ("P", "0"), vertical mask artifacts and label fragments even
    # when they do not look enough like a full text row. At the same time, it
    # protects insect legs/wings/antennae because they are either part of the same
    # connected component or are extremely close to a protected component. If a
    # text fragment physically touches the object, we keep it rather than risk
    # cutting the medical/biological object; that rare case should be handled by
    # a later AI refine fallback.
    for c in comps:
        if id(c) in protected_ids:
            continue
