...
def _style_lock_text(task: dict[str, Any], payload: dict[str, Any]) -> str:
    ...
    return f"""
STRICT SHARED TEMPLATE LOCK:
- This item belongs to visual_template_group={group.get('group_id')} similarity={similarity}.
- Reference PNG/entity: {ref}.
- Preserve the shared visual template of the group.
- Keep composition, object base geometry, scale, line thickness, palette, lighting, sharpness and detail level.
- Change only the variable distinguishing feature required for this item.
- Invariant features: {invariants or 'composition, scale, palette, line/detail level'}.
- Variable features: {variables or 'symptom/detail/stage/secondary object'}.
- Do not redesign, do not invent a new style, do not make it blurrier or glossier than the reference.
""".strip()
        ...
        for prompt in prompts:
            try:
                ...
                cost_items.append(cost_for_image_generation(
                    ...
                    image_count=1,
                    size=f"{api_side}x{api_side}",
                    metadata={"asset_id": asset_id, "png_id": png_id, "path": str(path), "quality_strategy": quality_strategy, "target_side": target_side, "image_quality": getattr(settings, "openai_image_quality", "low")},
                ))
                count += 1
                last_exc = None
                break
