...
def _content_pack(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for card in bp.get("cards") or []:
        if isinstance(card, dict):
            primary_png_id = card.get("primary_png_id") or card.get("png_id")
            secondary_png_id = card.get("secondary_png_id")
            png_ids = card.get("png_ids") if isinstance(card.get("png_ids"), list) else []
            if primary_png_id and primary_png_id not in png_ids:
                png_ids = [primary_png_id] + png_ids
            if secondary_png_id and secondary_png_id not in png_ids:
                png_ids.append(secondary_png_id)
            cards.append({
                "card_id": card.get("card_id"),
                "entity_id": card.get("entity_id"),
                "png_id": card.get("png_id") or primary_png_id,
