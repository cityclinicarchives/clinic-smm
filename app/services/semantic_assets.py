...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path, diagnostics: list[dict[str, Any]] | None = None) -> bool:
    """Extract a Semantic PNG using translate-first clipping correction.

    v43.5 extraction architecture:
    1) Start from the analyzer bbox exactly as provided, without global margin.
    2) If foreground touches a crop edge, move the crop window by 5% toward the
       touched side instead of expanding it. This fixes slightly shifted bboxes
       without pulling neighboring infographic items into the crop.
    3) For hero_visual, keep multiple large detached components.
    4) Only if translation cannot solve clipping, use scale fallback with small
       5% margin steps.
    """
    ...
    def process_box(box: tuple[int, int, int, int]) -> tuple[Image.Image, dict[str, bool], bool, dict[str, Any], bool, str]:
        crop = source_image.crop(box)
        if bool(task.get('transparent_background', True)):
            edge_clean = _remove_flat_corner_background(crop)
            touch = _alpha_edge_touch_sides(edge_clean, margin=4, min_opaque=8)
            clipped = any(touch.values()) and _box_can_expand(box, source_image.size)
            ...
            return cleaned, touch, clipped, metrics, suspicious, reason
        metrics = _foreground_quality_metrics(crop)
        suspicious, reason = _is_suspicious_extraction(metrics, preserve_multiple_components=preserve_multi)
        return crop, {"left": False, "right": False, "top": False, "bottom": False}, False, metrics, suspicious, reason
    ...
    def remember_candidate(cleaned: Image.Image, metrics: dict[str, Any], box_used: tuple[int, int, int, int], suspicious: bool, reason: str, stage: str) -> None:
    ...
    # Translate-first retry. Keep crop size stable; correct analyzer bbox drift.
    for _ in range(8):
        if box in seen_boxes:
            break
        seen_boxes.add(box)
        local_diag["translate_attempts"] += 1
        cleaned, touch, clipped, metrics, suspicious, reason = process_box(box)
        remember_candidate(cleaned, metrics, box, suspicious, reason, "translate")
        # Accept only if object no longer touches crop edge and output is not
        # suspicious. This avoids "successfully" saving a neighboring object or
        # a tiny text fragment simply because it is not clipped.
        if not clipped and not suspicious:
            path.parent.mkdir(parents=True, exist_ok=True)
            cleaned.save(path, 'PNG')
            local_diag.update({"final_box": [int(v) for v in box], "metrics": metrics})
        ...
        if shifted == box:
            break
        local_diag["shift_steps"].append({"touch": touch, "from": [int(v) for v in box], "to": [int(v) for v in shifted]})
        box = shifted

    # Scale fallback: only after shifting fails. Use gentle 5% steps so neighboring
    # objects are less likely to enter the crop than in the old 20→28→36→46% mode.
    for ratio in (0.05, 0.10, 0.15, 0.20, 0.25):
        expanded = _expand_crop_box(box, source_image.size, task, expansion_ratio=ratio)
        cleaned, touch, clipped, metrics, suspicious, reason = process_box(expanded)
        remember_candidate(cleaned, metrics, expanded, suspicious, reason, f"scale_{ratio:.2f}")
        if not clipped and not suspicious:
            path.parent.mkdir(parents=True, exist_ok=True)
            cleaned.save(path, 'PNG')
            local_diag.update({"scale_fallback": f"{int(ratio*100)}%", "final_box": [int(v) for v in expanded], "metrics": metrics})
            if diagnostics is not None:
                diagnostics.append(local_diag)
            return True
    ...
    if best_diag:
        local_diag.update({
            "scale_fallback": str(best_diag.get("stage") or "best_effort"),
            "final_box": best_diag.get("box") or [],
            "metrics": best_diag.get("metrics") or {},
            "suspicious": bool(best_diag.get("suspicious")),
            "suspicious_reason": str(best_diag.get("suspicious_reason") or ""),
        })
    if diagnostics is not None:
        diagnostics.append(local_diag)
    return True
...
def generate_semantic_pngs(asset_id: int, limit: int | None = None) -> tuple[list[str], list[str], dict[str, Any], dict[str, Any]]:
    ...
    for task in tasks:
        ...
        if operation == "extract_from_source" and quality_strategy in {"preserve_original_resolution", "extract_no_upscale", ""}:
            if _extract_source_png(task, source_image, path, stats.get("extraction_diagnostics")):
