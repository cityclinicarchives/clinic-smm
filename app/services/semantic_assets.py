...
def compose_reconstruction(asset_id: int) -> str:
    ...
    card_h = max(112, (available_h - gap_y * (rows - 1)) // rows)
    ...
    for idx, card in enumerate(cards):
        ...
        if examples:
            ex = " • ".join(examples[:5])
            # examples are source facts (e.g. drug/product names), rendered as a compact separate line
            short_text = (short_text + "\n" if short_text else "") + "Примеры: " + ex
