...
def _component_cleanup_and_trim(img: Image.Image) -> Image.Image:
    ...
    The heuristic keeps the main foreground component(s) and nearby insect/appendage
    details, while suppressing label fragments such as 'Common Ant' or 'Wasp'.
    It does not use OCR and does not assume a circular object.
    """
    ...
    keep: set[int] = set()
    # Always keep the largest foreground component.
    keep.add(id(main))
    min_meaningful_area = max(45, int(total_area * 0.012))
    near_distance = max(22, int(min(w, h) * 0.10))
    for c in comps[1:]:
        dist = bbox_distance(main_bbox, c['bbox'])
        bw, bh, area = c['bw'], c['bh'], c['area']
        # Text tends to be small, dark, horizontally elongated, detached and near crop bands.
        horizontal_text_like = c['dark_ratio'] > 0.45 and bw > bh * 1.8 and bh < max(34, int(h * 0.12))
        edge_band = c['bbox'][1] < h * 0.18 or c['bbox'][3] > h * 0.82
        tiny_detached = area < min_meaningful_area and dist > near_distance
        if horizontal_text_like and (edge_band or dist > near_distance // 2):
            continue
        if tiny_detached:
            continue
        # Preserve insect wings/legs or detached parts if close enough to the main object.
        if dist <= near_distance or area >= max(min_meaningful_area, int(total_area * 0.035)):
            keep.add(id(c))
