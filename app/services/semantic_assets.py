...
def _expand_crop_box(box: tuple[int, int, int, int], image_size: tuple[int, int], task: dict[str, Any], expansion_ratio: float | None = None) -> tuple[int, int, int, int]:
    ...
    if expansion_ratio is not None:
        ...
        ratio = 0.12 if confidence >= 0.75 else 0.16
    pad_x = max(12, int(crop_w * ratio))
    pad_y = max(12, int(crop_h * ratio))
    return (
        max(0, left - pad_x),
        max(0, top - pad_y),
...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path) -> bool:
    """Extract a Semantic PNG from the source image with crop expansion + clipping retry.

    The model bbox is treated as an approximate hint, not as a final crop. We
    expand it first and retry with wider margins if the foreground still touches
    the crop edge after background removal. This prevents clipped insects, wings,
    circle outlines and other thin details.
    """
    if source_image is None:
        return False

    # Progressive expansion ratios. The first value is already safer than v42.2;
    # later retries are used only when edge contact suggests possible clipping.
    expansion_ratios = [0.12, 0.18, 0.26, 0.36]
    best: Image.Image | None = None

    for ratio in expansion_ratios:
        box = _normalized_crop_box(task, source_image, expansion_ratio=ratio)
        if box is None:
            continue
        crop = source_image.crop(box)
        if bool(task.get('transparent_background', True)):
            edge_clean = _remove_flat_corner_background(crop)
            clipped = _alpha_touches_edge(edge_clean, margin=4, min_opaque=8) and _box_can_expand(box, source_image.size)
            cleaned = _remove_text_like_edge_fragments(edge_clean)
        else:
            clipped = False
            cleaned = crop
        best = cleaned
...
def compose_reconstruction(asset_id: int) -> str:
    ...
    header_x = 60
    header_y = 26
    hero_box_w = 210 if hero_paths else 0
    header_w = width - 120 - (hero_box_w + 24 if hero_paths else 0)
    header_h = 138
    ...
    footer_h = 128 if smart_blocks else 155
    footer_bottom = 38
    start_y = 188
    # v43.2: reserve a compact zone for useful smart blocks. The more cards,
    # the more compact the block area becomes. Composer may drop low-priority
    # blocks, but it must never drop cards.
    if smart_blocks:
        if len(cards) >= 14:
            smart_h = 104
        elif len(cards) > 10:
    ...
    available_h = max(260, smart_y - start_y - 18)
    card_w = (width - margin_x * 2 - gap_x * (cols - 1)) // cols

    # If the inferred grid cannot fit all cards, extend rows rather than
    # silently dropping cards. Then allocate row heights dynamically: rows with
    # long short_text/examples[] get more space, rows with short cards get less.
    max_cards = cols * rows
    ...
    for idx, card in enumerate(cards):
        ...
        # v43.5: examples[] are rendered as a dedicated compact line. They are
        # core source information and must not be swallowed by short_text.
        examples_h = 0
        if examples_text:
            examples_h = 34 if cols <= 2 else 28
        body_box_h = max(16, cy + card_h - body_y - 12 - examples_h)
        _draw_fitted_text(
            draw,
            (text_x, body_y),
            short_text,
