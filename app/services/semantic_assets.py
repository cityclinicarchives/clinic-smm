...
def _is_multi_component_extract_task(task: dict[str, Any]) -> bool:
    ...
    v43.5 diagnostic hotfix:
    - hero visuals are always multi-component capable;
    - ordinary PNGs become multi-component only when must_include contains
      several concrete physical parts, not when it only lists style details
      like highlight/outline/color.
    """
    ...
    style_or_quality_terms = (
        "цвет", "оттен", "блик", "контур", "фон", "градиент", "ярк",
        "тень", "глянец", "подпись", "текст", "край", "форма", "масштаб",
        "красн", "пятно", "отек", "отёк", "покрас", "папул", "реакц",
        "кольцо", "зона", "точк", "узор", "полоса", "метка",
    )
    ...
    for item in must_include:
        ...
        if not text:
            continue
        if any(term in text for term in style_or_quality_terms):
            continue
        physical_parts.append(text)
    return len(physical_parts) >= 2
