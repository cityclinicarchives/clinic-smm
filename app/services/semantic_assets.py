...
def _border_palette(img: Image.Image, max_colors: int = 4) -> tuple[list[tuple[int, int, int]], int]:
    ...
    # Conservative tolerance: do not eat light/white objects on warm paper backgrounds.
    tolerance = int(max(38, min(72, 42 + avg_dev * 0.30)))
...
def _restore_alpha_holes_from_original(original: Image.Image, cleaned: Image.Image) -> Image.Image:
    """Restore object interiors accidentally removed by background flood fill.

    v43.9 hotfix: light tubes/bottles can be close to the paper background color.
    A border-color flood fill may remove white highlights or tube body pixels inside
    the object. We restore only transparent regions that are fully enclosed by the
    remaining foreground alpha, so outer background stays transparent while holes
    inside the object come back.
    """
    try:
        orig = original.convert('RGBA')
        img = cleaned.convert('RGBA')
        w, h = img.size
        alpha = img.getchannel('A')
        px = img.load()
        opx = orig.load()
        # Flood-fill transparent pixels connected to the image border. Those are
        # real outside background. Transparent pixels not reached are inner holes.
        seen: set[tuple[int, int]] = set()
        q: deque[tuple[int, int]] = deque()
        for x in range(w):
            if alpha.getpixel((x, 0)) <= 12: q.append((x, 0))
            if alpha.getpixel((x, h - 1)) <= 12: q.append((x, h - 1))
        for y in range(h):
            if alpha.getpixel((0, y)) <= 12: q.append((0, y))
            if alpha.getpixel((w - 1, y)) <= 12: q.append((w - 1, y))
        while q:
            x, y = q.popleft()
            ...
            for nx, ny in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):
                if 0 <= nx < w and 0 <= ny < h and (nx, ny) not in seen and alpha.getpixel((nx, ny)) <= 12:
                    q.append((nx, ny))
        restored = 0
        max_restore = int(w * h * 0.35)
        for y in range(h):
            for x in range(w):
                if alpha.getpixel((x, y)) <= 12 and (x, y) not in seen:
                    r, g, b, a = opx[x, y]
                    if a > 12:
                        px[x, y] = (r, g, b, a)
                        restored += 1
...
def _dilate_mask(mask: Image.Image, size: int = 5, passes: int = 1) -> Image.Image:
    """Cheap morphology helper for binary L masks."""
    out = mask.convert('L')
...
def _erode_mask(mask: Image.Image, size: int = 3, passes: int = 1) -> Image.Image:
    out = mask.convert('L')
    size = max(3, int(size) | 1)
    for _ in range(max(1, passes)):
...
def _remove_contour_first_background(crop: Image.Image, preserve_multiple_components: bool = False) -> Image.Image:
    """Remove only pixels outside the detected object contour.

    v43.9 object-contour-first cleanup.

    The old background-color cleanup could erase white/light parts of an object
    (for example the body or highlights of a white tube) because those pixels
    were close to the paper background. This routine first builds a conservative
    object barrier from edges, dark text/contours and saturated pixels, closes the
    contour with cheap dilation, flood-fills the OUTSIDE from image borders, and
    then preserves everything inside the contour from the original crop. In other
    words: remove outside the object; do not recolor or erase object interiors.
    """
    try:
        ...
        palette, tolerance = _border_palette(original)
        px = original.load()

        # Build a conservative contour/barrier mask. We intentionally include
        # dark lettering on packages and saturated colored object pixels because
        # they help close the object contour. Background is usually pale and low
        # saturation, so it stays open to the border flood-fill.
        barrier = Image.new('L', (w, h), 0)
        bpx = barrier.load()
        edge_threshold = 16
        bg_tol = max(32, int(tolerance * 0.72))
        for y in range(h):
            for x in range(w):
                r, g, b, a = px[x, y]
                if a <= 12:
                ...
                dark = (r + g + b) < 500
                very_dark = (r + g + b) < 330
                different_from_bg = _min_color_distance((r, g, b), palette) > bg_tol
                edge = edges.getpixel((x, y)) >= edge_threshold
                if edge or very_dark or (dark and different_from_bg) or (sat > 34 and different_from_bg):
                    bpx[x, y] = 255

        # Close thin gaps in the visible outline. This is what makes white tube
        # interiors safe: once the outline is closed, the flood-fill cannot enter
        # and the whole interior is preserved from the original crop.
        barrier = _dilate_mask(barrier, size=5, passes=2)
        barrier = _erode_mask(barrier, size=3, passes=1)
        bpx = barrier.load()

        # Flood-fill outside area through non-barrier pixels.
        ...
        while q:
            ...
            if x < 0 or y < 0 or x >= w or y >= h:
                continue
            if opx[x, y] or bpx[x, y] > 0:
                continue
            opx[x, y] = 255
            for nx, ny in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):
                if 0 <= nx < w and 0 <= ny < h and not opx[nx, ny] and bpx[nx, ny] == 0:
                    q.append((nx, ny))

        out = original.copy()
        out_px = out.load()
        # Remove only outside. Keep all original pixels that are inside the
        # contour/barrier, including light highlights and white tube bodies.
        for y in range(h):
            for x in range(w):
                if opx[x, y] > 0:
                    r, g, b, a = out_px[x, y]
        ...
        # If the contour result is clearly empty or implausibly tiny, fall back
        # to the older edge-aware flood-fill. This preserves robustness for icons
        # without a closed contour.
        alpha = out.getchannel('A')
        bbox = alpha.getbbox()
        if not bbox:
            return _remove_edge_aware_background(crop)
        fg_area = sum(1 for v in alpha.getdata() if v > 18)
        if fg_area < max(80, int(w * h * 0.015)):
            return _remove_edge_aware_background(crop)
        return out
    except Exception:
        return _remove_edge_aware_background(crop)
    ...
    while q:
        ...
        for nx, ny in ((x+1,y),(x-1,y),(x,y+1),(x,y-1)):
            if 0 <= nx < w and 0 <= ny < h and (nx, ny) not in seen:
                q.append((nx, ny))
    img = _restore_alpha_holes_from_original(original, img)
    return img
...
def _component_cleanup_and_trim(img: Image.Image, preserve_multiple_components: bool = False) -> Image.Image:
    ...
    canvas = _post_clean_detached_text_artifacts(canvas, preserve_multiple_components=preserve_multiple_components)
    return canvas




def _post_clean_detached_text_artifacts(img: Image.Image, preserve_multiple_components: bool = False) -> Image.Image:
    """Final cheap cleanup for tiny detached text/watermark remnants.

    This runs after the main component cleanup. It is intentionally conservative:
    it removes only small, dark, detached components that look like letters,
    punctuation, watermark crumbs or UI specks. Colored pills, fruit, powder,
    shadows and object details are preserved. For hero/multi-component crops the
    thresholds are stricter so separate useful hero parts are not removed.
    """
    try:
        img = img.convert('RGBA')
        px = img.load()
        ...
        for sy in range(h):
            for sx in range(w):
                ...
                comps.append({
                    ...
                    'dark_ratio': dark_count / max(1, area),
                    'very_dark_ratio': very_dark_count / max(1, area),
                    'sat_ratio': sat_count / max(1, area),
                    'density': area / max(1, bw * bh),
                })
        ...
        delete: set[int] = set()
        for c in comps[1:]:
            area, bw, bh = int(c['area']), int(c['bw']), int(c['bh'])
            dist = bbox_distance(main['bbox'], c['bbox'])
            small = area <= max(260, int(total_area * (0.035 if not preserve_multiple_components else 0.018)))
            dark = c['dark_ratio'] > 0.45 or c['very_dark_ratio'] > 0.18
            low_color = c['sat_ratio'] < 0.35
            text_shape = (bh <= max(28, int(h * 0.16)) and bw >= max(4, int(bh * 0.45))) or area <= max(80, int(total_area * 0.006))
            detached = dist > max(2, int(min(w, h) * 0.015))
            near_outer_zone = c['bbox'][1] < h * 0.22 or c['bbox'][3] > h * 0.78 or c['bbox'][0] < w * 0.18 or c['bbox'][2] > w * 0.82
            if detached and small and dark and low_color and (text_shape or near_outer_zone):
                delete.add(id(c))

        if not delete:
            return img
        for c in comps:
            if id(c) not in delete:
                continue
            for x, y in c['pts']:
                r, g, b, a = px[x, y]
                px[x, y] = (r, g, b, 0)
...
def _main_foreground_bbox_from_alpha(alpha: Image.Image, preserve_multiple_components: bool = False) -> tuple[int, int, int, int] | None:
    """Return bbox of the main raw foreground object(s) in alpha coordinates."""
    try:
        a = alpha.convert("L")
        w, h = a.size
        if w <= 0 or h <= 0:
...
def _component_touch_sides_from_alpha(alpha: Image.Image, preserve_multiple_components: bool = False, margin: int = 3) -> dict[str, bool]:
    """Return gap-based edge contacts for the main raw foreground object(s).

    v43.6 Foreground Gap Crop Validation:
    touching means there is no sufficient background gap between the main
    foreground bbox and a crop side. Use a percentage gap (about 4% of the
    crop axis), not only a few opaque pixels at the border. This catches cases
    where a cropped object no longer has visible pixels exactly on the edge,
    but still has no safe background padding.
    """
    result = {"left": False, "right": False, "top": False, "bottom": False}
    try:
        a = alpha.convert("L")
        w, h = a.size
        bbox = _main_foreground_bbox_from_alpha(a, preserve_multiple_components=preserve_multiple_components)
        if not bbox or w <= 0 or h <= 0:
            return result
        x1, y1, x2, y2 = bbox
        min_gap_x = max(int(margin), int(round(w * 0.04)))
        min_gap_y = max(int(margin), int(round(h * 0.04)))
        left_gap = x1
        right_gap = max(0, w - 1 - x2)
        top_gap = y1
        bottom_gap = max(0, h - 1 - y2)
        result["left"] = left_gap < min_gap_x
        result["right"] = right_gap < min_gap_x
        result["top"] = top_gap < min_gap_y
        result["bottom"] = bottom_gap < min_gap_y
    except Exception:
        pass
    return result


def _foreground_gap_metrics_from_crop(crop: Image.Image, preserve_multiple_components: bool = False) -> dict[str, Any]:
    """Return foreground bbox gaps for diagnostics without affecting control flow."""
    try:
        alpha = _raw_foreground_alpha_from_border(crop).convert("L")
        w, h = alpha.size
        bbox = _main_foreground_bbox_from_alpha(alpha, preserve_multiple_components=preserve_multiple_components)
        if not bbox:
            return {"bbox": [], "gaps": {}, "min_gap": {"x": 0, "y": 0}}
        x1, y1, x2, y2 = bbox
        min_gap_x = max(3, int(round(w * 0.04)))
        min_gap_y = max(3, int(round(h * 0.04)))
        return {
            "bbox": [int(x1), int(y1), int(x2), int(y2)],
            "gaps": {
                "left": int(x1),
                "right": int(max(0, w - 1 - x2)),
                "top": int(y1),
                "bottom": int(max(0, h - 1 - y2)),
            },
            "min_gap": {"x": int(min_gap_x), "y": int(min_gap_y)},
        }
    except Exception:
        return {"bbox": [], "gaps": {}, "min_gap": {"x": 0, "y": 0}}
...
def _object_box_from_safe_search_region(
    source_image: Image.Image,
    search_box: tuple[int, int, int, int],
    preserve_multiple_components: bool = False,
) -> tuple[int, int, int, int] | None:
    """Localize the real foreground object inside analyzer's safe search box.

    v43.6 changes the contract: source_crop_hint.relative_box is a centered
    search region, not the final object crop. This helper finds foreground
    components inside that region and returns a tighter object box in source
    image coordinates. IMPORTANT: translate/scale validation must happen on
    the search region first; only after the search region has background gaps
    on all sides should this localized object box be used for final cleanup.
    """
    try:
        crop = source_image.crop(search_box).convert("RGBA")
        mask = _raw_foreground_alpha_from_border(crop).convert("L")
        w, h = mask.size
        if w < 10 or h < 10:
            return search_box
        px = mask.load()
        seen: set[tuple[int, int]] = set()
        comps: list[dict[str, Any]] = []
        min_area = max(18, int(w * h * 0.0015))
        for sy in range(h):
            for sx in range(w):
                if (sx, sy) in seen or px[sx, sy] <= 16:
                    continue
                ...
                aspect = bw / float(bh)
                # Filter obvious text-like strokes, but keep large labels printed
                # on bottles if they are part of the main package component.
                text_like = (aspect > 8.0 or aspect < 0.125) and area < max(120, int(w * h * 0.025))
                if text_like:
                    continue
                comps.append({"area": area, "bbox": (minx, miny, maxx + 1, maxy + 1)})
        if not comps:
            return search_box
        comps.sort(key=lambda c: c["area"], reverse=True)
        keep = [comps[0]]
        if preserve_multiple_components:
            min_keep = max(30, int(comps[0]["area"] * 0.16))
            keep.extend(c for c in comps[1:] if int(c["area"]) >= min_keep)
        x1 = min(c["bbox"][0] for c in keep)
        y1 = min(c["bbox"][1] for c in keep)
        x2 = max(c["bbox"][2] for c in keep)
        y2 = max(c["bbox"][3] for c in keep)
        # Add small internal padding so the derived crop is not tight. This is
        # not global expansion; it is a localized object padding inside the safe
        # search region, limited by that region's borders.
...
def _is_multi_component_extract_task(task: dict[str, Any]) -> bool:
    """Return True only when one PNG may intentionally contain detached objects.

    v43.6 Foreground Gap Crop Validation: ordinary card PNGs use single-object
    cleanup. Even if a card visually contains a bottle + fruit/spoon, Python
    treats the main connected visual group as one object and removes detached
    small components. Multi-component extraction is reserved for hero visuals.
    """
    ...
    return bool(role == "hero_visual" or "hero" in entity_id or png_id.startswith("hero") or mode == "hero_visual")
...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path, diagnostics: list[dict[str, Any]] | None = None) -> bool:
    ...
    def raw_gap_metrics_for_box(box: tuple[int, int, int, int]) -> dict[str, Any]:
        return _foreground_gap_metrics_from_crop(source_image.crop(box), preserve_multiple_components=preserve_multi)

    def cleanup_accepted_box(box: tuple[int, int, int, int]) -> tuple[Image.Image, dict[str, Any], bool, str]:
        # `box` is an accepted search box. Now, and only now, derive the tighter
        # object bbox inside it. This prevents an already-cropped object from
        # becoming the new validation frame and falsely passing gap checks.
        object_box = _object_box_from_safe_search_region(
            source_image,
            box,
            preserve_multiple_components=preserve_multi,
        ) or box
        crop = source_image.crop(object_box)
        ...
        metrics["search_box"] = [int(v) for v in box]
        metrics["object_box"] = [int(v) for v in object_box]
    ...
    search_box = _normalized_crop_box(task, source_image, expansion_ratio=0.0)
    if search_box is None:
        return False

    # v43.6 foreground-gap fix:
    # source_crop_hint.relative_box is a SAFE SEARCH BOX. Do not localize to a
    # tight object bbox before translate/scale validation, because that hides
    # the missing background gap and makes cropped objects look "valid". First
    # move/scale the search box until the raw foreground has background on all
    # sides. Only then derive the final object bbox inside that accepted search
    # box for cleanup and PNG saving.
    base_box = search_box
    local_diag["search_box"] = [int(v) for v in search_box]
    local_diag["base_box"] = [int(v) for v in base_box]
    local_diag["localization_mode"] = "gap_validate_search_box_then_localize"
    ...
    def accept_box(box: tuple[int, int, int, int], stage: str, raw_touch: dict[str, bool], scale_label: str = "") -> bool:
        ...
        base_left, base_top, base_right, base_bottom = base_box
        bw = max(1, base_right - base_left)
        bh = max(1, base_bottom - base_top)
        local_diag.update({
            "scale_fallback": scale_label,
            "translate_total_shift_percent": {
                "x": round(((box[0] - base_left) / float(bw)) * 100.0, 1),
                "y": round(((box[1] - base_top) / float(bh)) * 100.0, 1),
            },
            "final_box": [int(v) for v in box],
            "raw_touch": dict(raw_touch),
            "raw_gap_metrics": raw_gap_metrics_for_box(box),
    ...
    for attempt in range(6):  # base + up to five 5% shifts = 25% cumulative path
        ...
        local_diag["raw_gap_metrics"] = raw_gap_metrics_for_box(box)
    ...
    # 2) Scale fallback. Try both the final translated box and the original
    # analyzer bbox. If translation drifted toward a neighboring object, scaling
    # from the base bbox can recover a better crop. Each candidate expands in
    # repeated 5% steps and must pass RAW edge validation before cleanup.
    scale_origins: list[tuple[str, tuple[int, int, int, int]]] = [("translated", box)]
    if box != base_box:
        scale_origins.append(("base", base_box))
    for ratio in (0.05, 0.10, 0.15, 0.20, 0.25):
        for origin_name, origin_box in scale_origins:
            expanded = _expand_crop_box(origin_box, source_image.size, task, expansion_ratio=ratio)
            touch = raw_touch_for_box(expanded)
            step_record = {
                "origin": origin_name,
                "scale": f"{int(ratio*100)}%",
                "box": [int(v) for v in expanded],
                "raw_touch": touch,
                "raw_gap_metrics": raw_gap_metrics_for_box(expanded),
                "accepted": False,
            }
            if any(touch.values()):
                local_diag.setdefault("scale_steps", []).append(step_record)
                continue
            if accept_box(expanded, f"scale_{origin_name}_{ratio:.2f}", touch, f"{origin_name}+{int(ratio*100)}%"):
    ...
    debug_raw_path = debug_dir / f"{path.stem}_debug_raw.png"
    debug_cleaned_path = debug_dir / f"{path.stem}_debug_cleaned.png"
    ...
    # keep the artifact marked as debug/failed rather than success. All debug
    # files are generated from the same debug_box so raw/cleaned/last_attempt
    ...
    try:
        source_image.crop(debug_box).save(debug_raw_path, 'PNG')
    except Exception:
        pass
    ...
    try:
        cleaned.save(debug_cleaned_path, 'PNG')
    except Exception:
        pass
    ...
    base_left, base_top, base_right, base_bottom = base_box
    base_w = max(1, base_right - base_left)
    base_h = max(1, base_bottom - base_top)
    debug_left, debug_top, debug_right, debug_bottom = debug_box
    translate_total_shift_percent = {
        "x": round(((debug_left - base_left) / float(base_w)) * 100.0, 1),
        "y": round(((debug_top - base_top) / float(base_h)) * 100.0, 1),
    }
    ...
    local_diag.update({
        ...
        "translate_total_shift_percent": translate_total_shift_percent,
        "debug_path": str(debug_path),
        "debug_raw_path": str(debug_raw_path),
        "debug_cleaned_path": str(debug_cleaned_path),
        ...
        "raw_gap_metrics": raw_gap_metrics_for_box(debug_box),
        "cleanup_applied_after_crop_accept": False,
    })
    if diagnostics is not None:
        diagnostics.append(local_diag)
...
def _content_pack(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def normalize_cards(raw_cards: list[Any]) -> list[dict[str, Any]]:
        # v43.9 hotfix: preserve/infer multi-visual card links. The analyzer can
        # correctly create 14 PNG tasks for 7 paired rows, but if card links are
        # missing the Composer can only draw the primary PNG. This normalizer keeps
        # primary/secondary/png_ids and also infers the common pattern
        # comparison_item -> visual_explanation when needed.
        entity_order = [e for e in (payload.get("visual_entity_map") or []) if isinstance(e, dict)]
        entity_index = {str(e.get("entity_id") or ""): i for i, e in enumerate(entity_order)}
        png_by_entity: dict[str, str] = {}
        for task in payload.get("semantic_png_plan") or []:
            if isinstance(task, dict) and task.get("entity_id") and task.get("png_id"):
                png_by_entity[str(task.get("entity_id"))] = str(task.get("png_id"))

        out: list[dict[str, Any]] = []
        for card in raw_cards:
            if not isinstance(card, dict):
                continue
            raw_png_ids = card.get("png_ids") if isinstance(card.get("png_ids"), list) else []
            primary_png_id = card.get("primary_png_id") or card.get("png_id") or (raw_png_ids[0] if raw_png_ids else None)
            secondary_png_id = card.get("secondary_png_id") or (raw_png_ids[1] if len(raw_png_ids) > 1 else None)

            # Infer paired explanatory image if the next visual entity is a
            # visual_explanation PNG and the card did not explicitly link it.
            entity_id = str(card.get("entity_id") or "")
            if not secondary_png_id and entity_id in entity_index:
                i = entity_index[entity_id]
                if i + 1 < len(entity_order):
                    nxt = entity_order[i + 1]
                    if str(nxt.get("entity_role") or "") == "visual_explanation":
                        secondary_png_id = png_by_entity.get(str(nxt.get("entity_id") or ""))

            png_ids: list[str] = []
            for pid in [primary_png_id, secondary_png_id, *raw_png_ids]:
                pid = str(pid or "").strip()
                if pid and pid not in png_ids:
                    png_ids.append(pid)

            visuals = card.get("visuals") if isinstance(card.get("visuals"), list) else []
            out.append({
                "card_id": card.get("card_id"),
                "entity_id": card.get("entity_id"),
                "png_id": primary_png_id,
                "primary_png_id": primary_png_id,
                "secondary_png_id": secondary_png_id,
                "png_ids": png_ids,
                "visuals": visuals,
    ...
    if pack:
        ...
        normalized["cards"] = normalize_cards(pack.get("cards") if isinstance(pack.get("cards"), list) else [])
        return normalized

    # Fallback for old JSON files.
    bp = payload.get("design_blueprint") if isinstance(payload.get("design_blueprint"), dict) else {}
    cards = normalize_cards(bp.get("cards") if isinstance(bp.get("cards"), list) else [])
    return {
        ...
        if should_try_source_extract and quality_strategy in {"preserve_original_resolution", "extract_no_upscale", "", "redraw_from_reference", "hybrid", "try_extract_first"}:
            ...
            if isinstance(extraction_diags, list) and len(extraction_diags) > before_diag_count:
                ...
                if isinstance(last_diag, dict) and last_diag.get("status") == "debug_saved":
                    ...
                    for debug_key in ("debug_path", "debug_raw_path", "debug_cleaned_path"):
                        try:
                            dp = Path(str(last_diag.get(debug_key) or ""))
                            if dp.exists() and dp.stat().st_size > 0:
                                _register_artifact_safe(asset_id, state_id, "debug", dp)
                        except Exception:
                            pass
                    stats.setdefault("crop_debug_items", []).append({
                        ...
                        "debug_raw_path": str(last_diag.get("debug_raw_path") or ""),
                        "debug_cleaned_path": str(last_diag.get("debug_cleaned_path") or ""),
...
DESIGN_PRESETS = {
    "medical_soft": {
        ...
        "example_text": "#0B6FAE",
        "footer_bg": "#FFE7E3",
        "footer_accent": "#C83F3F",
        "warning_bg": "#FFF4D8",
        "warning_accent": "#D99000",
    },
    "medical_clean": {
        "bg": "#F7FBFF",
        "decor": "#E5F1FA",
    ...
    "warning_guide": {
        ...
        "footer_accent": "#C83F3F",
        "warning_bg": "#FFF1CC",
        "warning_accent": "#B56A00",
    },
}

def _choose_design_preset(payload: dict[str, Any], card_count: int) -> dict[str, str]:
    state = payload.get("analysis_state") if isinstance(payload.get("analysis_state"), dict) else {}
    asset_type = str(state.get("asset_type") or "").lower()
    topic = str(state.get("topic") or "").lower()
    custom = payload.get("custom") if isinstance(payload.get("custom"), dict) else {}
    preset_name = str(custom.get("design_preset") or "").lower()
    if preset_name in DESIGN_PRESETS:
        return DESIGN_PRESETS[preset_name]
    if any(x in topic for x in ["срочно", "к врачу", "укус", "опас", "клещ"]):
        return DESIGN_PRESETS["warning_guide"]
    if asset_type in {"checklist", "table"} or card_count >= 11:
        return DESIGN_PRESETS["checklist_cards"]
    if card_count <= 6:
        return DESIGN_PRESETS["medical_clean"]
    return DESIGN_PRESETS["medical_soft"]

def _draw_soft_background(draw: ImageDraw.ImageDraw, width: int, height: int, preset: dict[str, str]) -> None:
    # Cheap, deterministic decor: very light circles that add design without AI.
    # Presets may disable it for dense clinical layouts.
    if preset.get("decor_enabled", True) is False:
        return
    decor = preset.get("decor", "#F0E0C8")
    for box in [
        (-120, 180, 170, 470),
        (width - 160, 60, width + 90, 310),
        (width - 220, height - 360, width + 80, height - 60),
    ]:
        draw.ellipse(box, fill=decor)

def _detect_card_tag(card: dict[str, Any]) -> str:
    blob = (str(card.get("title") or "") + " " + str(card.get("short_text") or "") + " " + " ".join(map(str, card.get("examples") or []))).lower()
    if "2 час" in blob or "отдельно" in blob or "не вместе" in blob:
        return "2 часа" if "2 час" in blob else "не вместе"
    if "с ед" in blob or "жир" in blob or "завтрак" in blob or "обед" in blob:
        return "с едой"
    if "натощак" in blob:
        return "натощак"
    if "анализ" in blob or "феррит" in blob:
        return "анализ"
    if "врач" in blob or "назнач" in blob:
        return "по врачу"
    return ""

def _draw_pill_tag(draw: ImageDraw.ImageDraw, text: str, box: tuple[int, int, int, int], preset: dict[str, str]) -> None:
    if not text:
        return
    x1, y1, x2, y2 = box
    ...
    pad_x = 9
    tag_w = min(x2 - x1, tw + pad_x * 2)
    tag_h = 24
    tx2 = x2
    tx1 = tx2 - tag_w
    draw.rounded_rectangle((tx1, y1, tx2, y1 + tag_h), radius=tag_h//2, fill=preset.get("example_bg", "#EAF5FF"), outline=preset.get("card_border", "#DCEAF7"), width=1)
    draw.text((tx1 + pad_x, y1 + (tag_h - th)//2 - 1), text, fill=preset.get("primary", "#2F6F5E"), font=font)
...
def _smart_block_color(block: dict[str, Any], preset: dict[str, str] | None = None) -> dict[str, str]:
    role = str(block.get("color_role") or "info").lower()
    base = SMART_BLOCK_COLORS.get(role, SMART_BLOCK_COLORS["info"]).copy()
    if not preset:
        return base
    # Keep semantic background roles but pull accent/body from active design preset
    # so smart blocks do not look detached from card/header styling.
    if role in {"danger", "warning"}:
        base["accent"] = preset.get("footer_accent") or preset.get("warning_accent") or base["accent"]
    elif role in {"safe", "care"}:
        base["accent"] = preset.get("primary") or base["accent"]
    else:
        base["accent"] = preset.get("info") or preset.get("primary") or base["accent"]
    base["text"] = preset.get("body") or base["text"]
    return base


def _draw_smart_block(draw: ImageDraw.ImageDraw, block: dict[str, Any], box: tuple[int, int, int, int], preset: dict[str, str] | None = None) -> None:
    x1, y1, x2, y2 = box
    color = _smart_block_color(block, preset)
...
def _draw_smart_blocks_zone(draw: ImageDraw.ImageDraw, blocks: list[dict[str, Any]], x: int, y: int, w: int, h: int, preset: dict[str, str] | None = None) -> None:
    if not blocks or h <= 40:
        return
    n = len(blocks)
    gap = 16
    if n == 1:
        _draw_smart_block(draw, blocks[0], (x, y, x+w, y+h), preset)
    elif n == 2:
        bw = (w-gap)//2
        _draw_smart_block(draw, blocks[0], (x, y, x+bw, y+h), preset)
        _draw_smart_block(draw, blocks[1], (x+bw+gap, y, x+w, y+h), preset)
    else:
        top_h = (h-gap)//2
        bw = (w-gap)//2
        _draw_smart_block(draw, blocks[0], (x, y, x+bw, y+top_h), preset)
        _draw_smart_block(draw, blocks[1], (x+bw+gap, y, x+w, y+top_h), preset)
        _draw_smart_block(draw, blocks[2], (x, y+top_h+gap, x+w, y+h), preset)
...
def _card_png_ids(card: dict[str, Any]) -> list[str]:
    """Return primary/secondary PNG ids for a card.

    v43.8 Multi-Visual Cards: one content card may have more than one meaningful
    image, for example medicine tube + symptom circle. Keep `png_id` as the
    backward-compatible primary image and draw an optional secondary image when
    present.
    """
    ids: list[str] = []
    ...
    add(card.get("primary_png_id") or card.get("png_id"))
    add(card.get("secondary_png_id"))
    for pid in card.get("png_ids") or []:
        add(pid)
    for visual in card.get("visuals") or []:
        if isinstance(visual, dict):
            add(visual.get("png_id") or visual.get("id"))
    return ids[:3]
...
def _paste_png_fit(
    ...
    png_path: Path | None,
    box: tuple[int, int, int, int],
    fallback_fill: str = "#E9F5FF",
) -> bool:
    """Paste PNG into box preserving aspect ratio; return True on success."""
    x1, y1, x2, y2 = box
    w = max(1, x2 - x1)
    h = max(1, y2 - y1)
    if png_path and png_path.exists():
        try:
            ...
            p_img.thumbnail((w, h), Image.Resampling.LANCZOS)
            px = x1 + (w - p_img.width) // 2
            py = y1 + (h - p_img.height) // 2
...
def _draw_content_card(
    ...
    """
    preset = preset or DESIGN_PRESETS["medical_soft"]
    # Subtle card shadow + clean surface. This is cheap Python design, not AI.
    shadow = preset.get("card_shadow", "#EADDCB")
    draw.rounded_rectangle((x + 3, cy + 4, x + card_w + 3, cy + card_h + 4), radius=24, fill=shadow)
    draw.rounded_rectangle((x, cy, x + card_w, cy + card_h), radius=24, fill=preset.get("card_bg", "#FFFFFF"), outline=preset.get("card_border", "#DCEAF7"), width=2)
    png_ids = _card_png_ids(card)
    primary_path = pngs.get(png_ids[0]) if png_ids else None
    secondary_path = pngs.get(png_ids[1]) if len(png_ids) > 1 else None
    has_secondary = bool(secondary_path and secondary_path.exists())

    if horizontal and card_w >= 390:
        icon_box = min(132, max(78, card_h - 32), max(86, card_w // 3))
        secondary_box = min(72, max(44, icon_box // 2 + 8)) if has_secondary else 0
        icon_area_w = icon_box + (secondary_box + 8 if has_secondary and card_w >= 460 else 0)
        icon_x = x + 18
        icon_y = cy + max(12, (card_h - icon_box) // 2)
        text_x = x + icon_area_w + 38
        text_y = cy + 20
        text_w = max(120, card_w - icon_area_w - 60)
        title_h = max(30, min(58, int(card_h * 0.27)))
        title_max, title_min = 28, 15
        body_max, body_min = 21, 12
    else:
        icon_box = min(max(64, int(card_h * 0.34)), card_w - 28)
        secondary_box = min(58, max(38, icon_box // 2)) if has_secondary else 0
    ...
    # Image slot keeps card rhythm even when PNGs have very different silhouettes.
    slot_pad = max(4, icon_box // 18)
    primary_slot = (icon_x + slot_pad, icon_y + slot_pad, icon_x + icon_box - slot_pad, icon_y + icon_box - slot_pad)
    draw.rounded_rectangle(primary_slot, radius=max(12, icon_box//7), fill="#FFFFFF", outline=preset.get("card_border", "#DCEAF7"), width=1)
    if not _paste_png_fit(img, primary_path, primary_slot):
        draw.ellipse((icon_x, icon_y, icon_x + icon_box, icon_y + icon_box), fill="#E9F5FF", outline="#B8D6EE")

    # v43.8: optional secondary visual. Keep it visually subordinate so the card
    # still reads as one item, but preserve paired meaning such as medicine + symptom.
    if has_secondary:
        if horizontal and card_w >= 460:
            sx = icon_x + icon_box + 8
            sy = icon_y + max(0, (icon_box - secondary_box) // 2)
        else:
            sx = icon_x + icon_box - secondary_box + 2
            sy = icon_y + icon_box - secondary_box + 2
        secondary_slot = (sx, sy, sx + secondary_box, sy + secondary_box)
        draw.rounded_rectangle(secondary_slot, radius=max(10, secondary_box // 5), fill="#FFFFFF", outline=preset.get("card_border", "#DCEAF7"), width=1)
        _paste_png_fit(img, secondary_path, secondary_slot)
    ...
    _draw_fitted_text(
        ...
        max_size=body_max,
        min_size=body_min,
        fill=preset.get("body", "#444444"),
        bold=False,
        max_lines=None,
    )
    if examples_text:
        ex_y = cy + card_h - examples_h - 8
        # Dedicated examples chip: makes source examples visible without mixing
        # them into the main medical explanation.
        draw.rounded_rectangle((text_x - 2, ex_y - 2, text_x + text_w, ex_y + examples_h + 1), radius=10, fill=preset.get("example_bg", "#EAF5FF"))
        _draw_fitted_text(
            draw,
            (text_x + 5, ex_y + 1),
            ...
            fill=preset.get("example_text", "#0B6FAE"),
            bold=True,
            max_lines=2,
        )

    tag = _detect_card_tag(card)
    if tag and card_w >= 150 and card_h >= 180:
        _draw_pill_tag(draw, tag, (x + card_w - 92, cy + 12, x + card_w - 14, cy + 36), preset)
...
def compose_reconstruction(asset_id: int) -> str:
    ...
    cards_preview = pack.get("cards") if isinstance(pack.get("cards"), list) else []
    design_preset = _choose_design_preset(payload, len([c for c in cards_preview if isinstance(c, dict)]))
    img = Image.new("RGB", (width, height), design_preset.get("bg", "#FFF7E8"))
    draw = ImageDraw.Draw(img)
    _draw_soft_background(draw, width, height, design_preset)
    ...
    title_end_y = _draw_fitted_text(
        ...
        max_size=56,
        min_size=30,
        fill=design_preset.get("title", "#1E1E1E"),
        bold=True,
        max_lines=2,
    )
    _draw_fitted_text(
        ...
        max_size=26,
        min_size=16,
        fill=design_preset.get("subtitle", "#2F6F5E"),
        bold=False,
        max_lines=2,
    )
    if hero_paths:
    ...
    for card, x, cy, card_w, card_h, horizontal in card_boxes:
        _draw_content_card(img, draw, card, pngs, x, cy, card_w, card_h, horizontal=horizontal, preset=design_preset)

    if smart_blocks:
        _draw_smart_blocks_zone(draw, smart_blocks, 50, smart_y, width - 100, smart_h, design_preset)
    ...
    draw.rounded_rectangle((50, footer_y, width - 50, height - footer_bottom), radius=28, fill=design_preset.get("footer_bg", "#FFE7E3"))
    _draw_fitted_text(draw, (82, footer_y + 22), "!", 34, 50, 42, 24, fill=design_preset.get("footer_accent", "#C83F3F"), bold=True, max_lines=1)
    _draw_fitted_text(draw, (128, footer_y + 26), str(ftitle), width - 200, 38, 28, 16, fill=design_preset.get("footer_accent", "#C83F3F"), bold=True, max_lines=1)
    _draw_fitted_text(draw, (128, footer_y + 68), str(ftext), width - 190, footer_h - 78, 22, 12, fill=design_preset.get("body", "#1E1E1E"), bold=False, max_lines=4)
...
# v43.9 AI fallback reason report hotfix
def _build_ai_fallback_reason_report(local_diag: dict, reason: str) -> dict:
    return {
        "ai_fallback_reason": reason,
        "translate_attempts": local_diag.get("translate_attempts", 0),
        "scale_steps": local_diag.get("scale_steps", []),
        "raw_gap_metrics": local_diag.get("raw_gap_metrics", {}),
    }
