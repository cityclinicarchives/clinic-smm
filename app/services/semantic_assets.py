...
def _object_box_from_safe_search_region(
    ...
    image coordinates. Translate/scale validation then works on this localized
    box rather than on the whole search region.
    """
    try:
...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path, diagnostics: list[dict[str, Any]] | None = None) -> bool:
    ...
    if search_box is None:
        return False
    localized_box = _object_box_from_safe_search_region(
        source_image,
        search_box,
        preserve_multiple_components=preserve_multi,
    )
    base_box = localized_box or search_box
    local_diag["search_box"] = [int(v) for v in search_box]
    local_diag["base_box"] = [int(v) for v in base_box]
    local_diag["localization_mode"] = "safe_centered_search_box"
