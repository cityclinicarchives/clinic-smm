...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path, diagnostics: list[dict[str, Any]] | None = None) -> bool:
    ...
    # 3) Best-effort fallback, but keep diagnostics explicit. This should be rare
    # and visible in manifest so bad crops can be inspected.
    if best is None:
        # Try final cleanup of the current box so we have a file only if quality is not catastrophic.
        cleaned, metrics, suspicious, reason = cleanup_accepted_box(box)
        remember_candidate(cleaned, metrics, box, suspicious, reason, "best_effort_final", raw_touch_for_box(box))
    if best is None:
        return False
    path.parent.mkdir(parents=True, exist_ok=True)
    best.save(path, 'PNG')
    if best_diag:
        local_diag.update({
            ...
            "raw_touch": best_diag.get("raw_touch") or {},
            "cleanup_applied_after_crop_accept": True,
        })
    if diagnostics is not None:
        diagnostics.append(local_diag)
