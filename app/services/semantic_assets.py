...
def _remove_flat_corner_background(crop: Image.Image) -> Image.Image:
    """Simple transparent-background cleanup for beige/white screenshot backgrounds.

    It preserves the native crop resolution and avoids AI regeneration. This is
    intentionally conservative: only pixels close to the corner background color
    become transparent.
    """
    img = crop.convert('RGBA')
    pixels = img.load()
    w, h = img.size
    # Average the four corners as background sample.
...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path) -> bool:
    ...
    # Do not upscale. Save native crop resolution. Optionally remove flat background.
