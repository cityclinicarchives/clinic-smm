...
def _extract_source_png(task: dict[str, Any], source_image: Image.Image | None, path: Path) -> bool:
    ...
    expansion_ratios = [0.20, 0.28, 0.36, 0.46]
    ...
    for ratio in expansion_ratios:
        ...
        if bool(task.get('transparent_background', True)):
            ...
            cleaned = _remove_text_like_edge_fragments(edge_clean, preserve_multiple_components=_is_multi_component_extract_task(task))
        else:
            clipped = False
            cleaned = crop
        best = cleaned
