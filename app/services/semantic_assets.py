...
def _openai_square_size_for_target(target_side: int) -> int:
    """Map target size to conservative OpenAI square sizes.

    The API may not support arbitrary 270x270/360x360 requests. We request a
    nearby supported square and then downscale locally to the exact target.
    """
    if target_side <= 320:
        return 256
    if target_side <= 512:
...
Style: clean vector-like medical illustration, soft gradients, transparent-looking background, calm clinic design, square icon.
""".strip()
        ...
        if operation == "extract_from_source" and quality_strategy in {"preserve_original_resolution", "extract_no_upscale", ""}:
            if _extract_source_png(task, source_image, path):
                ...
                cost_items.append(free_operation(
                    ...
                    {"asset_id": asset_id, "png_id": png_id, "path": str(path), "quality_strategy": quality_strategy, "target_side": target_side},
                ))
                count += 1
                continue
        ...
        for prompt in prompts:
            try:
                ...
                cost_items.append(cost_for_image_generation(
                    ...
                    image_count=1,
                    size=f"{api_side}x{api_side}",
                    metadata={"asset_id": asset_id, "png_id": png_id, "path": str(path), "quality_strategy": quality_strategy, "target_side": target_side},
                ))
                count += 1
                last_exc = None
                break
