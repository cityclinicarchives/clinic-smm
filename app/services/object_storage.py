...
def object_exists(storage_key: str) -> bool:
    """Return True only when an object physically exists in R2.

    Missing object => False. Other R2/API errors are re-raised so callers do not
    silently treat a temporary storage failure as a valid cache miss.
    """
    if not storage_key or not r2_configured():
        return False
    client = _client()
    try:
...
def delete_object(storage_key: str) -> bool:
    """Delete one object from R2. Returns False when R2 is disabled/unconfigured or key is empty."""
    if not storage_key or not r2_configured():
        return False
    client = _client()
